    ...
    public partial class XSFPROD_STOCK : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        ...
        protected override void DataLoad()
        {
            ...
            else if (Constraint == "XFORD1200") //제품출고디테일 추가 시
            {
                ...
                // 20210823 오세완 차장 아직 재고가 남은 제품들이 출력이 안되는 현상이 있어서 수정
                //bindingSource.DataSource = ModelService.GetList(p => (string.IsNullOrEmpty(itemCode) ? true : p.ItemCode == itemCode)
                //                                                  && (string.IsNullOrEmpty(productLotNo) ? true : p.ProductLotNo == productLotNo)
                //                                                  && p.StockQty > 0
                //                                               )
                ...
                bool bFirst = false;
                List<VI_PROD_STOCK_PRODUCT_LOT_NO> detailArr = new List<VI_PROD_STOCK_PRODUCT_LOT_NO>();
                if(TN_ORD1201List != null)
                {
                    if (TN_ORD1201List.Count > 0)
                    {
                        detailArr = ModelService.GetList(p => (string.IsNullOrEmpty(itemCode) ? true : p.ItemCode == itemCode)
                                                           && (string.IsNullOrEmpty(productLotNo) ? true : p.ProductLotNo == productLotNo)
                                                           && p.StockQty > 0).Where(p => TN_ORD1201List.Select(c => c.ProductLotNo).Contains(p.ProductLotNo)).OrderBy(p => p.ItemCode).ThenBy(p => p.ProductLotNo).ToList();
                    }
                    else
                        bFirst = true;
                }
                ...
                if(bFirst)
                {
                    detailArr = ModelService.GetList(p => (string.IsNullOrEmpty(itemCode) ? true : p.ItemCode == itemCode)
                                                       && (string.IsNullOrEmpty(productLotNo) ? true : p.ProductLotNo == productLotNo)
                                                       && p.StockQty > 0).OrderBy(p => p.ItemCode).ThenBy(p => p.ProductLotNo).ToList();
                }

                bindingSource.DataSource = detailArr;
            }
