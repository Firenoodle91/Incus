...
namespace HKInc.Ui.View.View.ORD
{
    ...
    public partial class XFORD1200 : HKInc.Service.Base.ListMasterDetailFormTemplate
    {
        ...
        private void AddDetailCallBack(object sender, HKInc.Utils.Common.PopupArgument e)
        {
            ...
            {
                var detailObj = DetailGridBindingSource.List as List<TN_ORD1201>;

                var chk = detailObj.Where(p => p.ProductLotNo == v.ProductLotNo).ToList();
                if (chk.Count > 0)
                {
                    //이미 등록된 LOTNO  입니다.
                    MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_176), ""));
                }
                else
                {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        var chk = detailObj.Where(p => p.ProductLotNo == productLotNo).ToList();
                        if(chk.Count > 0)
                        {
                            MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_176), ""));
                            textEdit.EditValue = "";
