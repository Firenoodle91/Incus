...
namespace HKInc.Ui.View.View.POP
{
    ...
    /// POP
    /// </summary>
    public partial class XFPOP1000_RUS : HKInc.Service.Base.ListFormTemplate
    {
        #region 전역변수
        ...
        {
            btn_Search.Text = "найти";      // 조회
            //btn_RestartTO.Text = LabelConvert.GetLabelText("RestartTO"); // 20210602 오세완 차장 재가동TO
            btn_WorkStart.Text = "Начните работу";      // 작업시작
            btn_ResultAdd.Text = "Оформление выполнения работ";     // 실적등록
            btn_QualityAdd.Text = "проверка качества";      // 품질검사
            btn_WorkEnd.Text = "конец работы";      // 작업종료
            btn_MachineStop.Text = "нерабочий";     // 비가동
            btn_MachineCheck.Text = "Осмотр оборудования";      // 설비점검
            //btn_ItemMovePrint.Text = LabelConvert.GetLabelText("ItemMovePrint");
            btn_Exit.Text = "конец";        // 종료
        ...
        protected override void InitGrid()
        {
            ...
            gridEx1.MainGrid.AddColumn("JobStates", "рабочий статус");      // 작업상태
            gridEx1.MainGrid.AddColumn("EmergencyFlag", LabelConvert.GetLabelText("EmergencyFlag"), false);
            gridEx1.MainGrid.AddColumn("WorkNo", "номер рабочей инструкции");       // WorkNo
            gridEx1.MainGrid.AddColumn("WorkDate", "Рабочий день", HorzAlignment.Center, FormatType.DateTime, "yyyy -MM-dd");       // WorkDate
            gridEx1.MainGrid.AddColumn("ProcessCode", "процесс");       // Process
            gridEx1.MainGrid.AddColumn("MachineCode2", "оборудование");     // Machine
            gridEx1.MainGrid.AddColumn("ItemCode", "предметы");     // ItemCode
            gridEx1.MainGrid.AddColumn(DataConvert.GetCultureDataFieldName("ItemName"), "предметы");        // ItemName

            gridEx1.MainGrid.AddColumn("WorkQty", "Количество заданий", HorzAlignment.Far, FormatType.Numeric, "#,#.##");       // WorkQty 지시작업수량
            ...
            gridEx1.MainGrid.AddColumn("Memo", "записка");      // Memo
            ...
            gridEx2.MainGrid.AddColumn("ToolCode", "имя инструмента", HorzAlignment.Center, true);      // 공구명
            gridEx2.MainGrid.AddColumn("LifeCNT", "Оставшийся срок службы инструмента", HorzAlignment.Far, FormatType.Numeric, "#,0.##");       // 남은수명
        ...
        {
            SetMessage("★Аварийные работы отмечены красным цветом.★");        // ★긴급 작업은 빨간색으로 표시됩니다.★
            DataLoad();
        }
        ...
        private void RowChange()
        {
            ...
            if (obj.ProcessSeq == 1)
            {
                lcItemMoveNoEnd.Text = "Последний номер перемещения";      // 마지막 이동표 번호
            ...
            {
                lcItemMoveNoEnd.Text = "Текущий номер мобильного билета";      // 현 이동표 번호
        ...
        private void SetButtonEnable(string jobStates)
        {
            ...
            else
            {
                ...
                if (jobStates == MasterCodeSTR.JobStates_Wait) //대기
                {
                    ...
                    btn_WorkStart.Text = "Начните работу";      // LabelConvert.GetLabelText("WorkStart");
                    btn_ResultAdd.Enabled = false;
                    btn_QualityAdd.Enabled = false;
                    btn_WorkEnd.Enabled = false;
                    btn_MachineStop.Enabled = false;
        ...
        private void WorkStartCallback(object sender, PopupArgument e)
        {
            ...
            try
            {
                ...
                {
                    MessageBoxHandler.Show("productLotNo SP проверка", "제목");       // 확인
                    return;
                }
            }
            catch (Exception ex)
        ...
        private void Btn_QualityAdd_Click(object sender, EventArgs e)
        {
            ...
            var inspectionForm = new POP_POPUP.XPFQCIN_RUS(obj, tx_ProductLotNo.EditValue.GetNullToEmpty()); //JI버전
        ...
        private void Btn_MachineStop_Click(object sender, EventArgs e)
        {
            ...
            var stopForm = new POP_POPUP.XPFMACHINESTOP_RUS(vWorkObj);
            stopForm.ShowDialog();
            ActRefresh();
            LogFactory.GetMenuEventService().UpdateMenuEventLog(9999, "barButtonAdd");
        ...
        {
            var machineCheckForm = new POP_POPUP.XPFMACHINECHECK_RUS();
