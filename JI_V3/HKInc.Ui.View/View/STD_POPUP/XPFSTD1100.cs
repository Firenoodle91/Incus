...
namespace HKInc.Ui.View.View.STD_POPUP
{
    ...
    {
        IService<TN_STD1100> Std1100Service = (IService<TN_STD1100>)ProductionFactory.GetDomainService("TN_STD1100");
        IService<TN_UPH1000> UPH1000Service = (IService<TN_UPH1000>)ProductionFactory.GetDomainService("TN_UPH1000");
        private IService<VI_STD1100> VIStd1100Service;
        ...
        public XPFSTD1100(PopupDataParam param, PopupCallback callback)
        {
            ...
            spin_SrcWeight.EditValueChanged += Weight_EditValueChanged;
            spin_Weight.EditValueChanged += Weight_EditValueChanged;

            tx_ItemCode.KeyUp += Tx_ItemCode_KeyUp;
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("MachineMCode", LabelConvert.GetLabelText("MachineCode"), true);
            ...
            GridExControl.MainGrid.AddColumn("Uph", "UPH", HorzAlignment.Far, true);
         //   GridExControl.MainGrid.SetEditable(UserRight.HasEdit, "MachineMCode", "Uph", "ProcessCode");
        ...
        protected override void InitRepository()
        {
            ...
            GridExControl.MainGrid.SetRepositoryItemSearchLookUpEdit("MachineMCode", UPH1000Service.GetChildList<TN_MEA1000>(p => true), "MachineMCode", DataConvert.GetCultureDataFieldName("MachineName"));

            GridExControl.MainGrid.SetRepositoryItemSpinEdit("Uph", DefaultBoolean.True, "n0", true);
            lup_MC.SetDefault(true, "MachineMCode", DataConvert.GetCultureDataFieldName("MachineName"), UPH1000Service.GetChildList<TN_MEA1000>(p => p.UseFlag == "Y"));
        ...
        protected override void AddControlList()
        {
            ...
            //ControlEnableList.Add("MoldCode", lup_MoldCode);
            ControlEnableList.Add("Scrap", spin_Scrap);
            ControlEnableList.Add("Texture", lup_SrcTexture);
        ...
        protected override void InitBindingSource()
        {
            ...
            //Std1100Service = (IService<VI_STD1100>)PopupParam.GetValue(PopupParameter.Service);

            VIStd1100Service = (IService<VI_STD1100>)PopupParam.GetValue(PopupParameter.Service);
            //Std1100Service = (IService<TN_STD1100>)PopupParam.GetValue(PopupParameter.Service);
            //var obj = (VI_STD1100)PopupParam.GetValue(PopupParameter.Service);

            //Std1100Service.Insert() = 

            //var itemobj = Std1100Service.GetList(p => p.ItemCode == obj.ItemCode).ToList();

            //td1100Service.bi = itemobj;
        }

        protected override void InitCombo()
        {
            tx_ItemCode.ReadOnly = true;
            //tx_SrcTexture.ReadOnly = true;
            ...
            spin_Weight.Properties.Mask.EditMask = "n3";
            ...
            spin_SafeQty.Properties.Mask.EditMask = "n2";
            ...
            spin_SrcWeight.Properties.Mask.EditMask = "n3";
            ...
            spin_Scrap.Properties.Mask.MaskType = MaskType.Numeric;
            spin_Scrap.Properties.Mask.EditMask = "n3";
            spin_Scrap.Properties.Mask.UseMaskAsDisplayFormat = true;
            spin_Scrap.Properties.Buttons[0].Visible = false;
            ...
            lup_SrcCode.SetDefault(false, true, "ItemCode", DataConvert.GetCultureDataFieldName("ItemName"), Std1100Service.GetList(p => p.UseFlag == "Y" && p.TopCategory == MasterCodeSTR.TopCategory_MAT).ToList());
            ...
            //lup_MoldCode.SetDefault(true, "MoldMCode", DataConvert.GetCultureDataFieldName("MoldName"), Std1100Service.GetChildList<TN_MOLD1100>(p => p.UseYN == "Y").ToList());
            lup_SrcTexture.SetDefault(true, "CodeVal", DataConvert.GetCultureDataFieldName("CodeName"), DbRequestHandler.GetCommTopCode(MasterCodeSTR.SrcTexture));
        ...
        {
            Std1100Service.ReLoad();
            UPH1000Service.ReLoad();
            if (EditMode == PopupEditMode.New) // 신규 추가
            {
                var newObj = new TN_STD1100()
                {
                    ...
                    //ProcInspFlag = "Y",
                    //ShipmentInspFlag = "Y",
                    ItemCode = DbRequestHandler.GetSeqStandard("ITEM")
                    //FileFolderKeyName = DbRequestHandler.GetSeqDay("PROD_IMG")
                };

                ModelBindingSource.DataSource = newObj;

                ModelBindingSource.Add(newObj);
                ModelBindingSource.MoveLast();
                
                UPHbindingSource.DataSource = UPH1000Service.GetList(p => p.ItemCode == newObj.ItemCode).ToList();
                GridExControl.DataSource = UPHbindingSource;
            ...
            else
            {
                // Update
                tx_ItemCode.ReadOnly = true;
                var obj = (VI_STD1100)PopupParam.GetValue(PopupParameter.KeyValue);

                var itemobj = Std1100Service.GetList(p => p.ItemCode == obj.ItemCode).ToList();

                //btn_ProdFileName.Tag = obj.ProdFileUrl;
                //ModelBindingSource.DataSource = obj;
                ModelBindingSource.DataSource = itemobj;
                UPHbindingSource.DataSource = UPH1000Service.GetList(p => p.ItemCode == obj.ItemCode).ToList();
                GridExControl.DataSource = UPHbindingSource;
                GridExControl.MainGrid.BestFitColumns();
            }
        }
        ...
        protected override void DataSave()
        {
            ...
            if (EditMode == PopupEditMode.New)
            {
                ...
                //품명이 없고 고객사품명이 있을경우 품명을 phs 막음 2021.04.20
                //if (obj.ItemName.IsNullOrEmpty() && !obj.CustomerItemName.IsNullOrEmpty())
                //    obj.ItemName = obj.CustomerItemName;

                //품번(도번)이 없고, 고객사품번이 있을경우 phs 막음 2021.04.20
                ...
                //else
                //{
                //    string itemchk = obj.ItemCode.Substring(0, 5);
                ...
                //        if(obj.ItemCode.Length != 0)
                //        {

                //            MessageBox.Show("자릿수 10자리를 지켜주시기 바랍니다.");
                //            return;
                //        }

                //bool intchk = int.TryParse(obj.ItemCode.Substring(5, 5), out itemseq);

                //if (intchk)
                //{
                //    //return;

                //    string sSql = "USP_INS_ITEM_SEQ 'ITEM'," + itemseq + "";
            ...
            var tn_std1105 = Std1100Service.GetChildList<TN_STD1105>(p => p.ItemCode == obj.ItemCode).OrderByDescending(o => o.RowId).FirstOrDefault();
            if (tn_std1105 == null)
            {
        ...
        private void Lup_SrcCode_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (value.IsNullOrEmpty())
            {
                lup_SrcTexture.EditValue = null;
                spin_SrcWeight.EditValue = null;
            }
            else
            {
                lup_SrcTexture.EditValue = lookup.EditValue;
        ...
        private void Weight_EditValueChanged(object sender, EventArgs e)
        {
            ...
            VI_STD1100 obj2 = ModelBindingSource.Current as VI_STD1100;

            if (obj == null) return;

            string NAME = button.Name;
            decimal Weight = spin_Weight.EditValue.GetDecimalNullToZero();
            decimal SrcWeight = spin_SrcWeight.EditValue.GetDecimalNullToZero();
            decimal scrap = SrcWeight - Weight;


            //if (NAME == "spin_SrcWeight" && Weight > 0)
            //{
            //    spin_Scrap.EditValue = scrap;
            //    obj.Scrap = scrap;
            //}
            //else if(NAME == "spin_Weight" && Weight > 0 && SrcWeight > Weight)
            //{
            //    spin_Scrap.EditValue = scrap;
            //    obj.Scrap = scrap;
            //}
        ...
        private void SetControlEnableToTopCategory()
        {
            ...
            else if (topCategory == MasterCodeSTR.TopCategory_WAN || topCategory == MasterCodeSTR.TopCategory_BAN || topCategory == MasterCodeSTR.TopCategory_BAN_Outsourcing) // 20210524 오세완 차장 반제품을 자사 / 타사로 구분해서 타사를 추가처리
            {
                //완제품,반제품 선택
            ...
            else if (topCategory == MasterCodeSTR.TopCategory_MAT || topCategory == MasterCodeSTR.TopCategory_BU)
            {
                ...
                //--------------------------------------------
                lup_SrcTexture.ReadOnly = true;
                lup_MainMachineCode.ReadOnly = true;
                spin_Weight.ReadOnly = true;
                spin_Scrap.ReadOnly = true;

                lup_SrcTexture.EditValue = null;
                lup_MainMachineCode.EditValue = null;
                spin_Weight.EditValue = null;
                spin_Scrap.EditValue = null;
                //--------------------------------------------
            }
        }
        ...
        private void Tx_ItemCode_KeyUp(object sender, KeyEventArgs e)
        {
            if (EditMode == PopupEditMode.New) // 신규 추가
            {

                if (tx_ItemCode.Text.GetNullToEmpty() != "")
                {
                    var itemnmchk = Std1100Service.GetList(p => p.ItemCode == tx_ItemCode.Text.ToString()).ToList();

                    if (itemnmchk.Count > 0)
                    {
                        MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_41), LabelConvert.GetLabelText("ItemCode"), LabelConvert.GetLabelText("ItemCode")));
                        tx_ItemCode.Text = string.Empty;
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            var obj = ModelBindingSource.Current as TN_STD1100;
            string mc = lup_MC.EditValue.GetNullToEmpty();
            string proc = lup_process.EditValue.GetNullToEmpty();
            if ( mc== "") { MessageBox.Show("설비는 필수입니다."); return; }
            if (proc == "") { MessageBox.Show("공정은 필수입니다."); return; }
            //var oldobj = UPH1000Service.GetList(p => p.MachineMCode == mc && p.ProcessCode ==proc && p.ItemCode == obj.ItemCode).FirstOrDefault();
          
            List<TN_UPH1000> objl = UPHbindingSource.List as List<TN_UPH1000>;
            var oldobj = objl.Where(p => p.MachineMCode == mc && p.ProcessCode == proc && p.ItemCode == obj.ItemCode).FirstOrDefault();
            if (oldobj != null)
            {
                UPHbindingSource.Remove(oldobj);
                UPH1000Service.Delete(oldobj);
            }
            TN_UPH1000 NewDobj = new TN_UPH1000()
                {
                    MachineMCode = lup_MC.EditValue.GetNullToEmpty(),
                    ItemCode = obj.ItemCode,
                    Uph = tx_uph.EditValue.GetIntNullToZero(),
                    ProcessCode = lup_process.EditValue.GetNullToEmpty()
                };

                UPHbindingSource.Add(NewDobj);
                UPH1000Service.Insert(NewDobj);
            
            lup_MC.EditValue = "";
            lup_process.EditValue = "";
            tx_uph.EditValue = "";

            //UPHbindingSource.MoveLast();
        }
        protected  void simpleButton2_Click(object sender, EventArgs e)
        {
            var obj = UPHbindingSource.Current as TN_UPH1000;
            UPHbindingSource.RemoveCurrent();
            UPH1000Service.Delete(obj);
        ...
        private void MainView_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.Clicks >= 2) {
                var obj = UPHbindingSource.Current as TN_UPH1000;
                lup_MC.EditValue = obj.MachineMCode;
                lup_process.EditValue = obj.ProcessCode;
                tx_uph.EditValue = obj.Uph;
            }
        }
    }
}