...
namespace HKInc.Ui.View.View.POP
{
    ...
    public partial class XFPOP1000 : HKInc.Service.Base.ListFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            // 20220313 오세완 차장 권박사님이 컬러 출력 요구하여 추가 
            gridEx1.MainGrid.AddColumn("Temp1", LabelConvert.GetLabelText("ColorCode"));
            // 20220314 오세완 차장 컬러명 추가 
            gridEx1.MainGrid.AddColumn("ColorName", LabelConvert.GetLabelText("ColorName"));
            gridEx1.MainGrid.AddColumn("Temp2", LabelConvert.GetLabelText("ColorNickname"));
        ...
        private void Btn_WorkStart_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //    TN_MPS1201_NewObj.ItemMoveNo = prvObj.ItemMoveNo;
                //}
        ...
        private void WorkEndCallback(object sender, PopupArgument e)
        {
            ...
            if (endWorkInfo.TopCategoryCode == MasterCodeSTR.TopCategory_BAN)
            {
                ...
                //tN_BAN1001.InQty = tx_ResultQty.EditValue.GetDecimalNullToZero();
                ...
                ModelService.InsertChild<TN_BAN1000>(tN_BAN1000);
                ModelService.InsertChild<TN_BAN1001>(tN_BAN1001);
                ModelService.Save();
            }
        ...
        private List<string> AutoOutSrc(string srcItemCode, decimal srcUseQty, string workNo)
        {
            ...
            using (var context = new Model.Context.ProductionContext(ServerInfo.GetConnectString(GlobalVariable.ProductionDataBase)))
            {
                ...
                if (vResult != null && vResult.Count > 0)
                {
                    foreach (var s in vResult)
                    {
                        TN_PUR1301 newdtlobj = new TN_PUR1301()
                        {
                            ...
                            InLotNo = s.IN_LOT_NO,
                            //InCustomerLotNo = s.InCustomerLotNo,
                            Memo = srcItemCode + "자동차감출고",
                        };
                        ...
                        //else if (srcUseQty - calQty <= s.STOCK_QTY.GetDecimalNullToZero())
                        else
                        {
                            //newdtlobj.OutQty = s.STOCK_QTY.GetDecimalNullToZero() - (srcUseQty - calQty);
                ...
                else if (vResult.Count == 0)
                {
                    ...
                    //updateObj.OutQty = updateObj.OutQty + (srcUseQty - calQty);
                    //ModelService.UpdateChild(updateObj);
                    //rtnOutLotNo.Add(updateObj.OutLotNo);
