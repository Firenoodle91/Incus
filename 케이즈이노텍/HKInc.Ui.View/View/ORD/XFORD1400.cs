...
namespace HKInc.Ui.View.View.ORD
{
    ...
    public partial class XFORD1400 : HKInc.Service.Base.ListMasterDetailFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            DetailGridExControl.MainGrid.AddColumn("Temp", LabelConvert.GetLabelText("SortPackingJudge")); // 20211224 오세완 차장 정렬포장 컬럼 추가
            DetailGridExControl.MainGrid.AddColumn("Memo", LabelConvert.GetLabelText("Memo"));
            DetailGridExControl.MainGrid.SetEditable(UserRight.HasEdit, "_Check", "OutLotNo", "OutQty", "Memo", "PositionCode", "WhCode");
        ...
        protected override void InitRepository()
        {
            ...
            DetailGridExControl.MainGrid.SetRepositoryItemCheckEdit("Temp", "N");  // 20211224 오세완 차장 정렬포장 컬럼 추가
        ...
        private void BarButtonPrint_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                var detailList = DetailGridBindingSource.List as List<TN_ORD1401>;
                var printList = detailList.Where(p => p._Check == "Y").ToList();

                if (printList.Count == 0) return;

                //var mainReport = new REPORT.XRPACK();
                var mainReport = new REPORT.XRPACK_V3(); 
                foreach (var v in printList.OrderByDescending(p => p.OutSeq).ToList())
                {
                    //var report = new REPORT.XRPACK(v, obj);
                    var report = new REPORT.XRPACK_V3(v, obj); // 20211224 오세완 차장 변경된 라벨 양식 적용
