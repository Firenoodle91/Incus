...
namespace HKInc.Ui.View.SELECT_POPUP
{
    ...
    public partial class XSFORD1100 : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("DelivQty", LabelConvert.GetLabelText("DelivQty"), HorzAlignment.Far, FormatType.Numeric, MasterCodeSTR.Numeric_N2);

            if (Constraint == "XFPUR1600")
            {
                GridExControl.MainGrid.AddColumn("TurnKeyRemainQty", LabelConvert.GetLabelText("TurnKeyRemainQty"), HorzAlignment.Far, FormatType.Numeric, MasterCodeSTR.Numeric_N2);
            }
            if (Constraint == "OutConfirmFlag")
            {
                GridExControl.MainGrid.AddColumn("OutRemainQty", LabelConvert.GetLabelText("OutRemainQty"), HorzAlignment.Far, FormatType.Numeric, MasterCodeSTR.Numeric_N2);
            }
            ...
            GridExControl.MainGrid.AddColumn("TN_ORD1001.CustomerWorkNo", LabelConvert.GetLabelText("CustomerWorkNo")); // 20220210 오세완 차장 이태식 차장 요청으로 수주 엑셀업로드시 참조한 데이터 출력 추가
            GridExControl.MainGrid.AddColumn("TN_ORD1001.CustomerOrderPartName", LabelConvert.GetLabelText("CustomerOrderPartName"));
        ...
        protected override void DataLoad()
        {
            ...
            {
                // 20220210 오세완 차장 제품출고처리 할때만 거래처 작업지시번호 조회 조건 추가 
                bindingSource.DataSource = ModelService.GetList(p => (string.IsNullOrEmpty(itemCode) ? true : p.ItemCode == itemCode)
                                                                  && (string.IsNullOrEmpty(customerCode) ? true : p.CustomerCode == customerCode)
                                                                  && (p.DelivDate >= dt_DelivDate.DateFrEdit.DateTime && p.DelivDate <= dt_DelivDate.DateToEdit.DateTime)
                                                                  && (p.OutConfirmFlag == "Y")
                                                                  && (string.IsNullOrEmpty(sCustomerworkno) ? true : p.TN_ORD1001.CustomerWorkNo.Contains(sCustomerworkno))
                                                               )
            ...
            //                                                   .ToList();
            //}
            else if (Constraint == "XFORD1101") //일별출고관리 추가 시
            {
