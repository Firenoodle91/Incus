...
namespace HKInc.Ui.View.View.ORD_POPUP
{
    ...
    public partial class XPFORD1000 : HKInc.Service.Base.ListEditFormTemplate
    {
        ...
        public XPFORD1000(PopupDataParam param, PopupCallback callback)
        {
            ...
            lup_OrderCustomerCode.EditValueChanged += Lup_OrderCustomerCode_EditValueChanged;

            this.Text = LabelConvert.GetLabelText("OrderMasterInfo");
        }
        ...
        {
            lup_OrderCustomerCode.SetDefault(false, "CustomerCode", DataConvert.GetCultureDataFieldName("CustomerName"), ModelService.GetChildList<TN_STD1400>(p => p.UseFlag == "Y" && (p.CustomerType == MasterCodeSTR.CustType_Sales || p.CustomerType == null)).ToList());
        ...
        protected override void DataLoad()
        {
            if (EditMode == PopupEditMode.New) // 신규 추가
            {
                ...
                var newObj = new TN_ORD1000()
                {
                    ...
                    OrderDueDate = DateTime.Today.AddMonths(1),
        ...
        private void Lup_OrderCustomerCode_EditValueChanged(object sender, EventArgs e)
        {
            var value = lup_OrderCustomerCode.EditValue.GetNullToEmpty();
            if (!value.IsNullOrEmpty())
            {
                var dataRow = lup_OrderCustomerCode.GetSelectedDataRow() as TN_STD1400;
                if (!dataRow.BusinessManagementId.IsNullOrEmpty())
                {
                    lup_OrderId.EditValue = dataRow.BusinessManagementId;
