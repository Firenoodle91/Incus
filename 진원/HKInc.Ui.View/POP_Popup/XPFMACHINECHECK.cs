...
namespace HKInc.Ui.View.POP_Popup
{
    ...
    public partial class XPFMACHINECHECK : Service.Base.ListFormTemplate
    {
        ...
        protected override void InitCombo()
        {
            ...
            /*
            if (userObj == null)
                lup_Machine.SetDefault(false, "MachineCode", "MachineName", ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseYn == "Y").ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
            else
                //lup_Machine.SetDefault(false, "MachineCode", DataConvert.GetCultureDataFieldName("MachineName"), ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseFlag == "Y" && p.ProcTeamCode == userObj.ProductTeamCode).ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
                lup_Machine.SetDefault(false, "MachineCode", "MachineName", ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseYn == "Y").ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
            */

            lup_Machine.SetDefault(false, "MachineCode", "MachineName", ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseYn == "Y").ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
            lup_Machine.SetFontSize(new Font("맑은 고딕", 12f));
            lup_CheckId.SetDefault(false, "LoginId", "UserName", ModelService.GetChildList<User>(p =>  p.Active == "Y"
           ...
           // lup_CheckId.SetDefault(false, "LoginId", "UserName", ModelService.GetChildList<User>(p => p.Active == "Y").ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
            ...
            repositoryItemGridLookUpEdit.DataSource = DbRequesHandler.GetCommTopCode(MasterCodeSTR.MachineCheckOX).ToList();
        ...
        protected override void InitRepository()
        {
            ...
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("CheckPosition", DbRequesHandler.GetCommTopCode(MasterCodeSTR.MachineCheckPosition), "CodeVal", "CodeName");
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("CheckList", DbRequesHandler.GetCommTopCode(MasterCodeSTR.MachineCheckList), "CodeVal","CodeName");
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("CheckWay", DbRequesHandler.GetCommTopCode(MasterCodeSTR.MachineCheckWay), "CodeVal","CodeName");
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("CheckCycle", DbRequesHandler.GetCommTopCode(MasterCodeSTR.CHECKCYCLE), "CodeVal","CodeName");
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("CheckStandardDate", DbRequesHandler.GetCommTopCode(MasterCodeSTR.MachineCheckStandardDate), "CodeVal","CodeName");
            gridEx1.MainGrid.SetRepositoryItemSearchLookUpEdit("Temp2", ModelService.GetChildList<User>(p => true).ToList(), "LoginId", "UserName");
            gridEx1.MainGrid.MainView.Columns["ManagementStandard"].ColumnEdit = new Service.Controls.CommentGridButtonEdit(gridEx1.MainGrid.MainView,false,TextEditStyles.Standard, 200);
        ...
        private void Search(string machineCode)
        {
            ...
            if (TN_MEA1000 != null)
            {
                //if (!TN_MEA1000.FileName.IsNullOrEmpty())
                //    pic_CheckPointImage.EditValue = FileHandler.FtpToByte(GlobalVariable.HTTP_SERVER + TN_MEA1000.FileName);
                if (!TN_MEA1000.FileName2.IsNullOrEmpty())
                {
                    pic_CheckPointImage.EditValue = TN_MEA1000.FileData2;
                }
            }
