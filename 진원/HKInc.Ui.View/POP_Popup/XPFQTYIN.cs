...
namespace HKInc.Ui.View.POP_Popup
{
    public partial class XPFQTYIN : XtraForm
    {
        ...
        public XPFQTYIN(TP_POPJOBLIST obj,string lotno)
        {
            ...
            gridEx2.MainGrid.AddColumn("StopCode", "비가동사유", HorzAlignment.Center, true);
            gridEx2.MainGrid.AddColumn("StopStarttime", "비가동시작시간", HorzAlignment.Center, true);
            gridEx2.MainGrid.AddColumn("StopEnddate", "비가동종료시간", HorzAlignment.Center, true);
            gridEx2.MainGrid.AddColumn("DdifM", "비가동분", HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n0");
            gridEx2.MainGrid.SetRepositoryItemSearchLookUpEdit("StopCode", DbRequesHandler.GetCommCode(MasterCodeSTR.STOPTYPE), "Mcode", "Codename");
            ...
            gridEx1.MainGrid.AddColumn("fail_type", "불량유형(Fail_Type)");
            gridEx1.MainGrid.AddColumn("fail_name", "불량유형(Fail_Name)");
            gridEx1.MainGrid.AddColumn("fail_qty", "불량수량(Fail_Qty)");
            lsobj = obj;
            ...
            lup_machine.SetDefault(true, "MachineCode", "MachineName", ModelService.GetChildList<TN_MEA1000>(p => p.UseYn == "Y").ToList());
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
           ...
           int badqty = tx_sumbad.EditValue.GetIntNullToZero();
            ...
            //금형타발수 처리
            DbRequesHandler.SetDataQury(string.Format("EXEC SET_MOLD_SHOT '{0}', '{1}', {2}", lsobj.ItemCode, lsobj.Process, SUMQTY.GetNullToZero()));
        ...
        private void simpleButton3_Click(object sender, EventArgs e)
        {
            ...
            if (row == 0)
            {
                ...
                {
                    fail_type = lup_fail.EditValue.GetNullToEmpty(),
                    fail_name = lup_fail.SelectedText,
                    fail_qty = Convert.ToInt32(tx_bad.Text)
                };
        ...
        {
            if (!HKInc.Utils.Common.GlobalVariable.KeyPad) return;
            FWORKTIME key = new FWORKTIME();
            key.ShowDialog();
            tx_minute.Text = key.returnval;
            //XFCKEYPAD keypad = new XFCKEYPAD();
            //keypad.ShowDialog();
        ...
        private void tx_totokqty_EditValueChanged(object sender, EventArgs e)
        {
        ...
        {
            //시간 등록 양식 불확실 하므로 키패드 팝업 무조건 출력
            //if (!HKInc.Utils.Common.GlobalVariable.KeyPad) return;
            FWORKTIME key = new FWORKTIME();
            key.ShowDialog();
        ...
        private void simpleButton5_Click(object sender, EventArgs e)
        {
            ...
            {
                //비가동 추가시 필수 입력 요소 확인
                if (lup_machine.EditValue.GetNullToEmpty() == "")
                {
                    MessageBox.Show("설비 미선택", "알림", MessageBoxButtons.OK);
                    return;
                }
                if (textEdit1.EditValue.GetNullToEmpty() == "")
                {
                    MessageBox.Show("비가동 시작 시간 미입력", "알림", MessageBoxButtons.OK);
                    return;
                }

                if (textEdit2.EditValue.GetNullToEmpty() == "")
                {
                    MessageBox.Show("비가동 종료 시간 미입력", "알림", MessageBoxButtons.OK);
                    return;
                }

                if (lup_stop.EditValue.GetNullToEmpty() == "")
                {
                    MessageBox.Show("비가동 유형 미선택", "알림", MessageBoxButtons.OK);
                    return;
                }
