...
namespace HKInc.Ui.View.ORD
{
    public partial class XFORD1000 : HKInc.Service.Base.ListMasterDetailFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            MasterGridExControl.MainGrid.AddColumn("_chk", "선택");
            ...
            MasterGridExControl.MainGrid.SetEditable(UserRight.HasEdit, "_chk");
        ...
        {
            MasterGridExControl.MainGrid.SetRepositoryItemCheckEdit("_chk", "N");
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            if (PrintList.Where(p => p._chk == "Y").ToList().Count == 0) return;


            foreach (var v in PrintList.Where(p => p._chk == "Y").OrderByDescending(p => p.CreateTime).ToList())
            {
                int r = DbRequesHandler.GetCellValue("select count(*) from   [TN_ORD1100T] where ORDER_NO = '" + v.OrderNo + "'",0).GetIntNullToZero();
                if (r >= 1) {
                    MessageBox.Show("수주번호  " + v.OrderNo + "는 이미 납품계획이 수립되었습니다.");
                }
                else
                {
                    int i = DbRequesHandler.SetDataQury("exec batchord '" + v.OrderNo + "'");
