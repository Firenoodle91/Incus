...
namespace HKInc.Main
{
    public partial class FAlam1 : HKInc.Service.Base.ListFormTemplate
    {
        ...
        {
            lupHh.SetDefault(true, "Mcode", "Codename", DbRequesHandler.GetCommCode(MasterCodeSTR.AlamHH));
            lupMm.SetDefault(true, "Mcode", "Codename", DbRequesHandler.GetCommCode(MasterCodeSTR.AlamMM));
        }
        ...
        {
            GridExControl.MainGrid.SetRepositoryItemLookUpEdit("Hh", DbRequesHandler.GetCommCode(MasterCodeSTR.AlamHH), "Mcode", "Codename");
            GridExControl.MainGrid.SetRepositoryItemLookUpEdit("Mm", DbRequesHandler.GetCommCode(MasterCodeSTR.AlamMM), "Mcode", "Codename");
      //  GridExControl.MainGrid.MainView.Columns["Memo"].ColumnEdit = new HKInc.Service.Controls.CommentGridButtonEdit(gridEx1, UserRight.HasEdit, DevExpress.XtraEditors.Controls.TextEditStyles.Standard);
        ...
        {
            ModelService.Save();
            int i = DbRequesHandler.SetDataQury("update tn_memo set memo ='" + memoEdit1.EditValue + "' where idx=1");
            this.Close();
        }
        protected override void DataLoad()
        {
         ...
         memoEdit1.Text=   DbRequesHandler.GetCellValue("select memo from tn_memo", 0).GetNullToEmpty();
        }

        private void simpleButton3_Click(object sender, EventArgs e)
        {
            TN_ALAM002 obj = new TN_ALAM002() { No = DbRequesHandler.GetRequestNumberNew("ALam"), Hh = lupHh.EditValue.GetNullToEmpty(), Mm = lupMm.EditValue.GetNullToEmpty(), Memo = tx_memo.EditValue.GetNullToEmpty() };
            GridBindingSource.Add(obj);
            ModelService.Insert(obj);
        }
