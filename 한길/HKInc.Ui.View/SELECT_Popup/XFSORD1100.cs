...
namespace HKInc.Ui.View.SELECT_Popup
{
    public partial class XFSORD1100 : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            gridEx1.MainGrid.AddColumn("ShipmentRemainQty", "출고가능수량", HorzAlignment.Far, FormatType.Numeric, "n0");            
            gridEx1.MainGrid.AddColumn("Temp", "수주처");
        ...
        protected override void DataLoad()
        {
            ...
            bindingSource.DataSource = ModelService.GetList(p => (p.DelivDate >= dp_date.DateFrEdit.DateTime.Date  && 
                                                                  p.DelivDate <= dp_date.DateToEdit.DateTime.Date) && 
                                                                  (string.IsNullOrEmpty(cust) ? true : p.Temp == cust) &&
                                                                  p.OutDate==null && 
                                                                  p.OutConfirmflag=="Y"
                                                            )
                                                            //.Where(p=>p.TN_ORD1200List.Count == 0)
                                                            .Where(p => p.ShipmentRemainQty > 0)
