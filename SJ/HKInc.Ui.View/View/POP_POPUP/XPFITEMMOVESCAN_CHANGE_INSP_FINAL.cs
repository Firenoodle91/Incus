...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    /// 이동표 교체 (최종검사)
    /// </summary>
    public partial class XPFITEMMOVESCAN_CHANGE_INSP_FINAL : Service.Base.PopupCallbackFormTemplate
    {
        ...
        public XPFITEMMOVESCAN_CHANGE_INSP_FINAL()
        {
            InitializeComponent();
        }

        public XPFITEMMOVESCAN_CHANGE_INSP_FINAL(PopupDataParam parameter, PopupCallback callback) : this()
        {
            this.PopupParam = parameter;
            this.Callback = callback;
            this.Text = LabelConvert.GetLabelText("ItemMoveChange");

            TEMP_XFPOP_INSP_FINAL = (TEMP_XFPOP_INSP_FINAL)parameter.GetValue(PopupParameter.KeyValue);
        ...
        protected override void InitCombo()
        {
            ...
            {
                tx_ItemMoveNo1.EditValue = TEMP_XFPOP_INSP_FINAL.ItemMoveNo;
                tx_WorkNo1.EditValue = TEMP_XFPOP_INSP_FINAL.WorkNo;
                tx_ProductLotNo1.EditValue = TEMP_XFPOP_INSP_FINAL.ProductLotNo;
            ...
            {
                var ds = DbRequestHandler.GetDataQury("EXEC USP_GET_ITEM_MOVE_NO_SCAN_START '" + TEMP_XFPOP_INSP_FINAL.WorkNo + "', '" + TEMP_XFPOP_INSP_FINAL.ItemMoveNo + "'," + TEMP_XFPOP_INSP_FINAL.ProcessSeq + "");
                tx_ItemMoveNo1.EditValue = TEMP_XFPOP_INSP_FINAL.ItemMoveNo;
        ...
        private void Tx_ItemMoveNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (itemMoveNo == TEMP_XFPOP_INSP_FINAL.ItemMoveNo)
                {
                ...
                if (TEMP_XFPOP_INSP_FINAL.ProcessSeq == 1)
                {
                    ds = DbRequestHandler.GetDataQury("EXEC USP_GET_ITEM_MOVE_NO_SCAN_START_PACK '" + TEMP_XFPOP_INSP_FINAL.ItemCode + "', '" + itemMoveNo + "'");
                }
                else
                {
                    ds = DbRequestHandler.GetDataQury("EXEC USP_GET_ITEM_MOVE_NO_SCAN_START '" + TEMP_XFPOP_INSP_FINAL.WorkNo + "', '" + itemMoveNo + "'," + TEMP_XFPOP_INSP_FINAL.ProcessSeq + "");
                }
                ...
                else if (TEMP_XFPOP_INSP_FINAL.ProcessSeq != 1 && ds.Tables[1].Rows[0][0].ToString() != TEMP_XFPOP_INSP_FINAL.WorkNo)
                {
