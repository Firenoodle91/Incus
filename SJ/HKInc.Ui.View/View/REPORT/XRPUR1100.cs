...
namespace HKInc.Ui.View.View.REPORT
{
    ...
    public partial class XRPUR1100 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        List<TN_STD1000> unitList = DbRequestHandler.GetCommTopCode(MasterCodeSTR.Unit);
        ...
        public XRPUR1100(TN_PUR1100 masterObj) : this()
        {
            ...
            cell_Unit.BeforePrint += cell_Unit_BeforePrint;
            ...
            if (masterObj != null)
            {
                ...
                cell_Memo.Text = masterObj.PoMonth.ToString("yyyy-MM") + "월 발주"; //특기사항
        ...
        private void cell_ItemCode_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (!itemCode.Trim().IsNullOrEmpty())
            {
                ...
                else
                {
                    ...
                    //{
                    //    //국문
                    //    cell_ItemCode.Text = itemObj.ItemCode.GetNullToEmpty() + " / " + itemObj.ItemName.GetNullToEmpty();
                    //}
                    //else if (DataConvert.GetCultureIndex() == 2)
                    //{
                    //    //영문
                    //    cell_ItemCode.Text = itemObj.ItemCode.GetNullToEmpty() + " / " + itemObj.ItemNameENG.GetNullToEmpty();
                    //}
                    //else if (DataConvert.GetCultureIndex() == 3)
                    //{
                    //    //중문
                    //    cell_ItemCode.Text = itemObj.ItemCode.GetNullToEmpty() + " / " + itemObj.ItemNameCHN.GetNullToEmpty();
                    //}
                    //if (!itemObj.CombineSpec.Trim().IsNullOrEmpty())
                    //    cell_ItemCode.Text += " / " + itemObj.CombineSpec;

                    cell_ItemCode.Text = itemObj.CustomerItemCode.GetNullToEmpty();
                }
            }
        }

        //단위
        private void cell_Unit_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            var value = cell_Unit.Text;
            if (!value.IsNullOrEmpty())
            {
                var obj = unitList.Where(p => p.CodeVal == value).FirstOrDefault();
                if (obj != null)
                    cell_Unit.Text = obj.CodeName;
                else
                    cell_Unit.Text = "";
            }
            else
