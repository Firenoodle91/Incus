...
namespace HKInc.Ui.View.View.REPORT
{
    ...
    public partial class XRQCT1001 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XRQCT1001(TN_ORD1201 obj) : this()
        {
            ...
            cell_CustomerName.Text = obj.TN_ORD1200.TN_STD1400.CustomerName.Length > 8 ? "고객사" : obj.TN_ORD1200.TN_STD1400.CustomerName;
            ...
            cell_DueDate.Text = obj.TN_ORD1200.OutReqDate == null ? obj.TN_ORD1200.OutDate.ToShortDateString() : ((DateTime)obj.TN_ORD1200.OutReqDate).ToShortDateString();
            ...
            if (inspectData != null)
            {
                ...
                if (inspectData.TN_QCT1501List.Count > 0)
                {
                    for (int n = 1; n < 8; n++)
                    {
                        ...
                        chktype[n-1] = listInst.TN_QCT1001.CheckDataType; //추가 phs 20201015                        
                    }
                    ...
                    int i = 0;
                    foreach (var v in inspectData.TN_QCT1501List.Where(p => (p.TN_QCT1001 != null && p.TN_QCT1001.InspectionReportFlag == "Y")).OrderBy(p => p.InspSeq).ToList())
                    {
                        if (i == 7) break;
                        ...
                        if (v.CheckDataType != "C") //검사방법-치수검사 //추가 20201015
                        {
                            //항목별 검사 값 합계
                            sumVal[i] = (v.Reading1.GetDecimalNullToZero() + v.Reading2.GetDecimalNullToZero() + v.Reading3.GetDecimalNullToZero()
                                        + v.Reading4.GetDecimalNullToZero() + v.Reading5.GetDecimalNullToZero() + v.Reading6.GetDecimalNullToZero()
                    ...
                    for (int a = 0; a < lblList1.Length; a++)
                    {
                        for (int b = 0; b < 7; b++)
                        {
                            ...
                            { 
                                ...
                                varCnt[b] += 1; //추가 20201015
                            }

                            //if (lblList1[a][b].Text.GetNullToEmpty() != "")
                            //    varCnt[b] += 1; //막음 20201015
                        }
                    }
                    ...
                    for(int x = 0; x < 7; x++) //검사항목 수 x
                    {
                        ...
                        {
                            //lblList2[0][x].Text = string.Format("{0:n3}", (sumVal[x].GetDecimalNullToZero() / varCnt[x].GetDecimalNullToZero())); //막음 phs 20201025
                            //phs 추가 20201015 검사규격 데이터타입의 숫자형에 +1만큼
                            if (chktype[x] == "0")
                            {
                                lblList2[0][x].Text = string.Format("{0}", (sumVal[x].GetDecimalNullToZero() / varCnt[x].GetDecimalNullToZero()));
                            }
                            else if (chktype[x] == "1")
                            {
                                lblList2[0][x].Text = string.Format("{0:n1}", (sumVal[x].GetDecimalNullToZero() / varCnt[x].GetDecimalNullToZero()));
                            }
                            else if (chktype[x] == "2")
                            {
                                lblList2[0][x].Text = string.Format("{0:n2}", (sumVal[x].GetDecimalNullToZero() / varCnt[x].GetDecimalNullToZero()));
                            }
                            else if (chktype[x] == "3")
                            {
                            ...
                            {
                                lblList2[0][x].Text = string.Format("{0:n4}", (sumVal[x].GetDecimalNullToZero() / varCnt[x].GetDecimalNullToZero()));
                            }
        ...
        public XRQCT1001(TN_QCT1500 obj) : this()
        {
            ...
            //cell_printDate.Text = DateTime.Today.ToShortDateString();
            cell_CustomerName.Text = obj.TN_STD1400.CustomerName.Length > 8 ? "고객사" : obj.TN_STD1400.CustomerName;
            cell_ItemCode.Text = obj.ItemCode;
            cell_ItemName.Text = cultureIndex == 1 ? obj.TN_STD1100.ItemName : (cultureIndex == 2 ? obj.TN_STD1100.ItemNameENG : obj.TN_STD1100.ItemNameCHN);
            ...
            if(obj.PrintDate != null)
                cell_DueDate.Text = ((DateTime)obj.PrintDate).ToShortDateString();
            ...
            if (obj.TN_QCT1501List.Count > 0)
            {
                ...
                foreach (var v in obj.TN_QCT1501List.Where(p => (p.TN_QCT1001 != null && p.TN_QCT1001.InspectionReportFlag == "Y")).OrderBy(p => p.InspSeq).ToList())
                {
                    ...
                    chktype[n-1] = v.CheckDataType;//추가 phs 20201015
                ...
                lblList1 = new XRLabel[][]
                {
                    ...
                    new XRLabel[] {  Type1X8, Type2X8, Type3X8, Type4X8, Type5X8, Type6X8, Type7X8}, //X8
                    new XRLabel[] {  Type1X9, Type2X9, Type3X9, Type4X9, Type5X9, Type6X9, Type7X9},  //X9
                    new XRLabel[] {  Type1X10, Type2X10, Type3X10, Type4X10, Type5X10, Type6X10, Type7X10},  //X10
                    new XRLabel[] {  Type1X11, Type2X11, Type3X11, Type4X11, Type5X11, Type6X11, Type7X11},  //X11
                    new XRLabel[] {  Type1X12, Type2X12, Type3X12, Type4X12, Type5X12, Type6X12, Type7X12},  //X12
                    new XRLabel[] {  Type1X13, Type2X13, Type3X13, Type4X13, Type5X13, Type6X13, Type7X13},  //X13
                    new XRLabel[] { Type1Judge, Type2Judge, Type3Judge, Type4Judge, Type5Judge, Type6Judge, Type7Judge},                      //판정
                };
                ...
                foreach (var v in obj.TN_QCT1501List.Where(p => (p.TN_QCT1001 != null && p.TN_QCT1001.InspectionReportFlag == "Y")).OrderBy(p => p.InspSeq).ToList())
                {
                    ...
                    lblList1[7][i].Text = v.Reading8;
                    lblList1[8][i].Text = v.Reading9;
                    lblList1[9][i].Text = v.Reading10;
                    lblList1[10][i].Text = v.Reading11;
                    lblList1[11][i].Text = v.Reading12;
                    lblList1[12][i].Text = v.Reading13;
                    lblList1[13][i].Text = v.Judge;

                    //if (v.CheckWay == MasterCodeSTR.InspectionWay_Input) //검사방법-치수검사
                    if (v.CheckDataType != "C") //검사방법-치수검사
                    {
                        //항목별 검사 값 합계
                        sumVal[i] = (v.Reading1.GetDecimalNullToZero() + v.Reading2.GetDecimalNullToZero() + v.Reading3.GetDecimalNullToZero()
                                    + v.Reading4.GetDecimalNullToZero() + v.Reading5.GetDecimalNullToZero() + v.Reading6.GetDecimalNullToZero()
                ...
                for (int a = 0; a < lblList1.Length; a++)
                {
                    for (int b = 0; b < 7; b++)
                    {
                        ...
                        //if (lblList1[a][b].Text.GetNullToEmpty() != "") //PHS 막음 20201015
                             //varCnt[b] += 1;
                    }
                }
