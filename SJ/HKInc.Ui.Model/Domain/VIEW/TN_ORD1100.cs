...
namespace HKInc.Ui.Model.Domain.VIEW
{
   ...
   [Table("TN_ORD1100T")]
    public class TN_ORD1100 : BaseDomain.MES_BaseDomain
    {
        ...
        /// <summary> 출고증에서 사용하는 잔량 </summary>
        public decimal RemainOutPlanQty
        {
            get
            {
                if (TN_ORD1101List.Count == 0) return DelivQty;
                else
                {
                    var outPlanQty = TN_ORD1101List.Sum(p => p.OutPlanQty);
                    return outPlanQty == null ? 0 : DelivQty - (decimal)outPlanQty;
                }
            }
        }
