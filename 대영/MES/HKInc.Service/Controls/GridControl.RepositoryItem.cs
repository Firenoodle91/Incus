...
namespace HKInc.Service.Controls
{
    partial class GridControlEx
    {
        ...
        public void SetRepositoryItemSearchLookUpEdit(string fieldName, object dataSource, string valueMember = "CodeVal", string displayMember = "CodeName", bool addDeleteButton = false, bool addEllipsisButton = false, ButtonPressedEventHandler ellipsisButtonPressedHandler = null, string nullText = "", EventHandler gridLookUpBeforePopupHandler = null, TextEditStyles textEditStyles = TextEditStyles.Standard)
        {
            ...
            else if(fieldName == "ProductItemCode")
            {
                ItemCodeColumnSetting(lookup, fieldName, valueMember, displayMember);
            }
            ...
            {
                // 20210928 오세완 차장 XFQCT1900에서 TN_QCT1901에 연계에서 사용해야 해서 추가 처리
                bool bExists_Dot = false;
                string[] sArr = fieldName.Split('.');
                if (sArr != null)
                    if (sArr.Length > 1)
                        bExists_Dot = true;
                ...
                if(bExists_Dot)
                {
                    ...
                    {
                        FieldName = valueMember,
                        Caption = HelperFactory.GetLabelConvert().GetLabelText(sArr[1]),
                        Visible = true //코드표시
                    });
