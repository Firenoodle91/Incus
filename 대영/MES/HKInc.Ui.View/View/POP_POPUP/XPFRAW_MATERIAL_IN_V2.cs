...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    /// </summary>
    public partial class XPFRAW_MATERIAL_IN_V2 : Service.Base.PopupCallbackFormTemplate
    {
        #region 전역변수
        ...
        public XPFRAW_MATERIAL_IN_V2()
        {
            InitializeComponent();
        }

        public XPFRAW_MATERIAL_IN_V2(PopupDataParam parameter, PopupCallback callback) : this()
        {
                ...
                if (data != null)
                {
                    if (data.DailyCheckFlag == "Y")
                    {
                        if (ModelService.GetChildList<TN_MEA1003>(p => p.MachineCode == data.MachineMCode && p.CheckDate == DateTime.Today).FirstOrDefault() == null)
                        {
                            ...
                            //lookup.EditValue = null; // 20220113 오세완 차장 작업자들이 일일점검 안해도 시작할 것 같아서 수정
                        }
                    }
                }
                                ...
                                if (each.ITEM_CODE == sInput_Itemcode)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (sAutoOutSrc == "Y")
                                        {
                                            ...
                                            sMessage = MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_164); // 20210702 오세완 차장 메시지 교체
                                            MessageBoxHandler.Show(sMessage);
                                            bBreak = true;
                                        }
                                        else
