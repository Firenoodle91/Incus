...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    public partial class XPFMOLDCHECK : Service.Base.ListFormTemplate
    {
        ...
        /// <summary>
        /// 20220110 오세완 차장 작업지시에 지정된 설비를 먼저 출력해 달라는 신부장님 요청으로 추가
        /// </summary>
        ...
        public XPFMOLDCHECK(TEMP_XFPOP1000 pWork)
        {
            InitializeComponent();
        ...
        protected override void InitRepository()
        {
            ...
            // 20220114 오세완 차장 프레스 공정으로부터 금형을 인식한 정보를 확인처리
            if (workObj != null)
                DataLoad();
        }
        ...
        {
            // 20220110 오세완 차장 프레스 pop에서 호출된 경우 작업지시 정보를 확인하여 금형코드를 알 수 있다. 
            if(workObj != null)
            {
                string sSql = "exec USP_GET_XPFMOLDCHECK_VALUE '" + workObj.WorkNo + "', '" + workObj.ProcessCode + "' ";
                DataSet ds = DbRequestHandler.GetDataQury(sSql);
                if(ds != null)
                    if(ds.Tables.Count > 0)
                    {
                        string sMold_mcode = ds.Tables[0].Rows[0]["WORK_MOLD_MCODE"].GetNullToEmpty();
                        tx_Mold.EditValue = sMold_mcode;
                    }
            }
        }
