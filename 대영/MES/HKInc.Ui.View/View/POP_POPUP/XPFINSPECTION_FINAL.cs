...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    public partial class XPFINSPECTION_FINAL : HKInc.Service.Base.ListFormTemplate
    {
        ...
        protected override void InitCombo()
        {
            ...
            btn_file.ButtonPressed += new HKInc.Service.Handler.EventHandler.FtpFileButtonEditClickHandler(true
                                                                                            , true
                                                                                            , btn_file
                                                                                            , true
                                                                                            , MasterCodeSTR.FtpFolder_Inspection_Final_File).ButtonClick;
        }
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("InspCheckPosition", LabelConvert.GetLabelText("InspCheckPosition"), false);
        ...
        protected override void DataLoad()
        {
            ...
            //                                                                        && (p.ProcessCode == TEMP_XFPOP_INSP_FINAL.ProcessCode || p.ProcessCode == null)
            //                                                                        && p.CheckDivision == MasterCodeSTR.InspectionDivision_Final
            //                                                                        && p.Temp1 == ItemMoveNo).FirstOrDefault();
        ...
        protected override void DataSave()
        {
            ...
            #region 실적INSERT
            /*
                        TN_MPS1201 MasterObj = QcModelService.GetChildList<TN_MPS1201>(p => p.WorkNo == TEMP_XFPOP_INSP_FINAL.WorkNo
                                                                            && p.ProcessCode == TEMP_XFPOP_INSP_FINAL.ProcessCode
                                                                            && p.ProcessSeq == TEMP_XFPOP_INSP_FINAL.ProcessSeq
                                                                            && p.ProductLotNo == ProductLotNo
                                                                            && p.ItemMoveNo == ItemMoveNo
                                                                        ).FirstOrDefault();

                        if (!MasterObj.TN_MPS1202List.Any(p => p.ItemMoveNo == ItemMoveNo))
                        {
                            TN_ITEM_MOVE ItemMoveObj = QcModelService.GetChildList<TN_ITEM_MOVE>(p => p.ItemMoveNo == ItemMoveNo && p.ProcessSeq == MasterObj.ProcessSeq - 1).FirstOrDefault();
                            //TN_ITEM_MOVE ItemMoveObj = QcModelService.GetChildList<TN_ITEM_MOVE>(p => p.ItemMoveNo == ItemMoveNo && p.ProcessSeq == MasterObj.ProcessSeq - 1).FirstOrDefault();

                            MasterObj.ResultSumQty += ItemMoveObj.ResultQty;
                            MasterObj.OkSumQty += ItemMoveObj.OkQty;
                            MasterObj.BadSumQty += ItemMoveObj.BadQty;
                            ...
                            detailNewObj.ResultQty = ItemMoveObj.ResultQty;
                            detailNewObj.OkQty = ItemMoveObj.OkQty;
                            detailNewObj.BadQty = ItemMoveObj.BadQty;
                            ...
                            {
                                checkItemMoveObj.ResultSumQty += ItemMoveObj.ResultSumQty;
                                checkItemMoveObj.OkSumQty += ItemMoveObj.OkSumQty;
                                checkItemMoveObj.BadSumQty += ItemMoveObj.BadSumQty;
                                checkItemMoveObj.ResultQty += ItemMoveObj.ResultQty;
                                checkItemMoveObj.OkQty += ItemMoveObj.OkQty;
                                checkItemMoveObj.BadQty += ItemMoveObj.BadQty;
            ...
            var TN_QCT1100_NewObj = new TN_QCT1100()
            {
                ...
                ProductLotNo = ProductLotNo,
                OkQty = TEMP_XFPOP_INSP_FINAL.OkQty,
                NgQty = TEMP_XFPOP_INSP_FINAL.BadQty,
                Temp1 = ItemMoveNo,
            ...
            //파일업로드
            if (btn_file.EditValue.IsNullOrEmpty()) TN_QCT1100_NewObj.FileUrl = null;
            else
            {
                string[] filename = btn_file.EditValue.ToString().Split('\\');
                if (filename.Length != 1)
                {
                    var realFileName = TN_QCT1100_NewObj.InspNo + "_" + filename[filename.Length - 1];
                    var ftpFileUrl = MasterCodeSTR.FtpFolder_Inspection_Final_File + "/" + realFileName;

                    FileHandler.UploadFTP(btn_file.EditValue.ToString(), realFileName, GlobalVariable.FTP_SERVER + MasterCodeSTR.FtpFolder_Inspection_Final_File + "/");

                    btn_file.EditValue = realFileName;
                    btn_file.Tag = null;
                    TN_QCT1100_NewObj.FileUrl = ftpFileUrl;
                    TN_QCT1100_NewObj.FileName = realFileName;
                }
            }
            ...
            foreach (var v in list)
            {
                var TN_QCT1101_NewObj = new TN_QCT1101()
                {
                    ...
                    InspCheckPosition = v.InspCheckPosition,
                    CheckMax = v.CheckMax,
                    CheckMin = v.CheckMin,
        ...
        private void CheckInput(TN_QCT1001 obj)
        {
            ...
            var checkUp = checkSpec + obj.CheckUpQuad.GetDecimalNullToZero();
            var checkDown = checkSpec - obj.CheckDownQuad.GetDecimalNullToZero();
            //var checkUp = obj.CheckUpQuad.GetDecimalNullToZero();
            //var checkDown = obj.CheckDownQuad.GetDecimalNullToZero();
