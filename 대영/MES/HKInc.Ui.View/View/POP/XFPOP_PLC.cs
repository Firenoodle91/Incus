...
namespace HKInc.Ui.View.View.POP
{
    ...
    /// 프레스 POP, plc와 통신하는 
    /// 20210721 오세완 차장 
    /// 메인 타이머를 사용한 버전이며 현장 plc와 간단 테스트 용으로 사용하고 실제 배포에는 사용하지 않기로 함
    /// </summary>
    public partial class XFPOP_PLC : HKInc.Service.Base.ListFormTemplate
    {
        #region 전역변수
        ...
        /// <summary>
        /// 202010707 오세완 차장
        /// 실적 갱신 시간 ini에서 가져오게 설정하기 위한 변수
        /// </summary>
        private int gi_RefreshTime = 1000;

        /// <summary>
        /// 20210709 오세완 차장
        /// 대영은 1초에 2~3회 이상 타발을 하기 때문에 실적 업데이트 주기를 정함
        /// </summary>
        private short gsi_Updatecnt = 0;
        #endregion
        ...
        public XFPOP_PLC()
        {
            ...
            // 20210707 오세완 차장 plc통신 타이머 설정 추가
            gTimer = new System.Timers.Timer(20000); 
            gTimer.Elapsed += GTimer_Elapsed_NotConnection; // 20200129 오세완 차장 test용
            gTimer.SynchronizingObject = this;
        ...
        private void WorkEndCallback(object sender, PopupArgument e)
        {
            ...
            // 20210712 오세완 차장 작업종료시에서 실적 초기화
            Send_Plc_Reset();
        ...
        private void SrcChangeCallback(object sender, PopupArgument e)
        {
            ...
            if (productLotNo != tx_ProductLotNo.EditValue.GetNullToEmpty())
            {
                ...
                // 20210712 오세완 차장 이동표가 바뀌면 plc실적 리셋한다. 
                Send_Plc_Reset();
            } 
        ...
        private void ItemMovePrintCallback(object sender, PopupArgument e)
        {
            ...
            // 20210614 오세완 차장 유미대리 요청으로 A4크기 부품이동표로 변경
            //var ItemMoveNoReport = new XRITEMMOVENO(masterObj, detailList);
        ...
        /// <summary>
        /// 20210707 오세완 차장 테스트로 개통을 확인할 때 사용
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void GTimer_Elapsed_NotConnection(object sender, System.Timers.ElapsedEventArgs e)
        {
            TEMP_XFPOP1000 tObj = GridBindingSource.Current as TEMP_XFPOP1000;
            if (tObj == null)
                return;
            ...
            string sMachine = "MC-00001";
            gsi_Updatecnt++;

            try
            {
                using (System.Net.Sockets.TcpClient tc1 = new System.Net.Sockets.TcpClient(sIPAddress, iPort))
                {
                    tc1.ReceiveTimeout = 5000; // 20200921 오세완 차장 무작정 수신을 대기하여 뻗는 것을 방지해보고자 추가
                    Modbus.Device.ModbusIpMaster master = Modbus.Device.ModbusIpMaster.CreateIp(tc1);

                    ushort startAddress = 0;
                    int iResult_Productcnt;

                    if (master != null)
                    {
                        iResult_Productcnt = master.ReadInputRegisters_int(startAddress);
                    }
                    else
                    {
                        return;
                    ...
                    tx_Count.BeginInvoke(new MethodInvoker(delegate ()
                    {
                        tx_Count.EditValue = iResult_Productcnt.ToString();
                    }));
                }
            ...
            {
                MessageBoxHandler.Show(ex_win32.ToString());
            }
            catch(Exception ex)
            {
        ...
        /// <summary>
        /// 20210707 오세완 차장 설비가 정상적으로 연결된 경우 사용
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void GTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (tObj == null)
                return;
            //else if (tObj.JobStates != MasterCodeSTR.JobStates_Start)
                //return;
            ...
            int iPort;
            int.TryParse(sPort, out iPort);
            string sMachine = IIniFileControl.ReadIniFile("PLCINFO", "MACHINE");
            gsi_Updatecnt++;
            ...
            try
            {
                using (System.Net.Sockets.TcpClient tc1 = new System.Net.Sockets.TcpClient(sIPAddress, iPort))
                {
                    ...
                    #region PLC와 연결되어 연결 상태값 변경
                    List<TN_MEA1600> plc_Arr = ModelService.GetChildList<TN_MEA1600>(p => p.Ip == sIPAddress && p.Port == iPort);
                    ...
                    if (plc_Each == null)
                    {
                        MessageBoxHandler.Show("설비 인터페이스 정보를 확인하시기 바랍니다.");
                        return;
                    }

                    plc_Each.Connection = "CONNECT";
                    #endregion

                    #region 실제 PLC로부터 실적 count를 받아오는 곳
                    ushort startAddress = 0;
                    int iResult_Productcnt;
                    ...
                    // 20210709 오세완 차장 db update 주기를 설정
                    if (gsi_Updatecnt > 30)
                    {
                        #region 생산카운트 증가시 PLC인터페이스 테이블 실적처리 
                        DateTime dtPrevTime;
        ...
        /// <summary>
        /// 20210707 오세완 차장
        /// plc 통신 설정파일을 읽어들여서 연결 초기화 및 타이머 동작
        /// </summary>
        private void Init_PLC_Connection()
        {
            string sIPAddress = IIniFileControl.ReadIniFile("PLCINFO", "IP");
            ...
            string sPort = IIniFileControl.ReadIniFile("PLCINFO", "PORT");
            int iPort;
            bool bCheckPort = int.TryParse(sPort, out iPort);

            string sMachine = IIniFileControl.ReadIniFile("PLCINFO", "MACHINE");
            if (sMachine != null)
                if (!sMachine.Equals(string.Empty))
                    lup_Machine.EditValue = sMachine;

            // 20200924 오세완 차장
            // 실적 생신시간 파일 입력 기능 추가
            int iRefreshTime;
            string sRefreshTime = IIniFileControl.ReadIniFile("PLCINFO", "REFRESH_TIME");
            bool bInput_Refresh = int.TryParse(sRefreshTime, out iRefreshTime);
            if (bInput_Refresh)
                gi_RefreshTime = iRefreshTime * 1000;

            if (bCheckIP && bCheckPort)
            {
                gTimer.Interval = gi_RefreshTime;
                gTimer.Enabled = true;
            }
        }
