        ...
        private void WorkEndCallback(object sender, PopupArgument e)
        {
            ...
            if (e.Map.ContainsKey(PopupParameter.Constraint)) //새출력 시 
            {
                ...
                using (var context = new HKInc.Ui.Model.Context.ProductionContext(HKInc.Utils.Common.ServerInfo.GetConnectString(HKInc.Utils.Common.GlobalVariable.ProductionDataBase)))
                {
                    ...
                    var vResult = context.Database.SqlQuery<TEMP_ITEM_MOVE_NO_MASTER>("USP_GET_ITEM_MOVE_NO_MASTER_V3 @WorkNo, @ItemMoveNo", _workNo, _ItemMoveNo).ToList(); // 20211123 오세완 차장 2공정 이후에도 이동표가 분할이 되는 버전 교체
                    if (vResult != null)
                ...
                using (var context = new HKInc.Ui.Model.Context.ProductionContext(HKInc.Utils.Common.ServerInfo.GetConnectString(HKInc.Utils.Common.GlobalVariable.ProductionDataBase)))
                {
                    ...
                    detailList = context.Database.SqlQuery<TEMP_ITEM_MOVE_NO_DETAIL>("USP_GET_ITEM_MOVE_NO_DETAIL_V2 @WorkNo, @ItemMoveNo", _workNo, _itemMoveNo).ToList(); // 20211125 오세완 차장 작업자가 제대로 표시되지 않아서 개선된 버전으로 사용
                }
                ...
                if (tObj.ProcessSeq == iProcessMax)
                {
                    ...
                    else
                    {
                        using (var context = new HKInc.Ui.Model.Context.ProductionContext(ServerInfo.GetConnectString(GlobalVariable.ProductionDataBase)))
                        {
                            ...
                            SqlParameter sp_Workno = new SqlParameter("@WORK_NO", tObj.WorkNo); // 20220106 오세완 차장 프로시저 변경으로 수정 
                                        ...
                                        if (purEach.TN_PUR1301List.Count > 0)
                                        {
                                            foreach (TN_PUR1301 each in purEach.TN_PUR1301List)
                                            {
                                                ...
                                                using (var context = new HKInc.Ui.Model.Context.ProductionContext(ServerInfo.GetConnectString(GlobalVariable.ProductionDataBase)))
                                                {
                                                    ...
                                                    SqlParameter sp_Workno = new SqlParameter("@WORK_NO", mstEach.WorkNo); // 20220106 오세완 차장 프로시저 수정으로 변경 
                                                    //var vResult = context.Database.SqlQuery<TEMP_XPFRESULT_BOMLIST_BANPRODUCT>("USP_GET_XPFRESULT_BOMLIST_BANPRODUCT_AUTO @ITEM_CODE", sp_Itemcode).ToList();
                                                    var vResult = context.Database.SqlQuery<TEMP_XPFRESULT_BOMLIST_BANPRODUCT>("USP_GET_XPFRESULT_BOMLIST_BANPRODUCT_AUTO @ITEM_CODE, @WORK_NO", sp_Itemcode, sp_Workno).ToList(); // 20220106 오세완 차장 프로시저 수정으로 변경 
                                                    if (vResult != null)
                                                        if (vResult.Count > 0)
                                                        {
