   ...
   public class DbRequestHandler
    {
       ...
       public static int GetRowCount(string lssql)
        ...
        public static List<TN_STD1000> GetCommCode(string codemain, string codemid, string codesub, string codeval)
        {
            ...
            {
                rtnvalue = ModelService.GetList(p => (p.Codemain == codemain) && (string.IsNullOrEmpty(codemid) ? p.Codemid != "00" : p.Codemid == codemid)
                                                && (p.Codesub == "00") && (p.Codeval == "00"))
                                                .OrderBy(p => (p.Displayorder)).ToList();
            }
            else if (codesub == "" && codeval == "")
            {
                rtnvalue = ModelService.GetList(p => (p.Codemain == codemain) && (p.Codemid == codemid) && (string.IsNullOrEmpty(codesub) ? p.Codesub != "00" : p.Codesub == codesub) && (p.Codeval == "00"))
                                              .OrderBy(p => (p.Displayorder)).ToList();
            }
            else if (codeval == "")
            {
                rtnvalue = ModelService.GetList(p => (p.Codemain == codemain) && (p.Codemid == codemid)
                                              && (p.Codesub == codesub) && (p.Codeval != "00"))
                                              .OrderBy(p => (p.Displayorder)).ToList();
            }
        ...
        public static List<TN_STD1000> GetCommCode(string codemain)
        {
            ...
            rtnvalue = ModelService.GetList(p => (p.Codemain == codemain) && (p.Useyn != "N") &&
           (p.Codemid != "00"))
                                            //                              && (p.Codesub == "00") && (p.Codeval == "00"))
                                            .OrderBy(p => (p.Displayorder)).ToList();
        ...
        /// 단가이력관리에 지정된 단가 가져오기
        /// </summary>
        /// <param name="pItemcode">품목코드</param>
        /// <param name="pCustomercode">거래처 코드</param>
        /// <param name="sDate">지정일자 yyyy-MM-dd</param>
        /// <returns>decimal </returns>
        public static decimal GetManagedCost(string pItemcode, string pCustomercode, string sDate)
        {
            decimal dReturn = 0m;
            ...
            {
                ProductionContext db = new ProductionContext(ServerInfo.GetConnectString(GlobalVariable.ProductionDataBase));
                dReturn = db.GetManagedCost(pItemcode, pCustomercode, sDate);
            }
            catch
            {
