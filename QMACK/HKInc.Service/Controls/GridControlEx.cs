...
{
    [ToolboxItem(true)]
    public partial class GridControlEx : DevExpress.XtraEditors.XtraUserControl
    {
            ...
            if (e.MenuType == GridMenuType.Column)
            {
                ...
                if (menu.Column != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(this.gridControl.Parent.Parent.Name))
                    {
                        ...
                        //menu.Items.Add(CreateGridLayoutSavePmenuItem(FieldNameConverter.GetLabelText("Grid_Layout_Pmenu_Save"), menu.Column));
                        //menu.Items.Add(CreateGridLayoutInitPmenuItem(FieldNameConverter.GetLabelText("Grid_Layout_Pmenu_Init"), menu.Column));
                    }
                }
            }
        }
        ...
        private void Grid_LayoutSave_Pmenu_Click(object sender, EventArgs e)
        {
            ...
            if (obj != null)
            {
                string Msg = string.Format("등록된 UI가 존재합니다. 덮어 쓰시겠습니까?");
                DialogResult result = Handler.MessageBoxHandler.Show(Msg, FieldNameConverter.GetLabelText("Confirm"), MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    ModelService.Save();
                    Handler.MessageBoxHandler.Show("UI가 저장 되었습니다.", "완료", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
        ...
        public void BestFitColumns(int MaxRowCount = 100)
        {
        ...
        public void SummaryItemAddNew(int colIndex)
        {
            //GridGroupSummaryItem item = new GridGroupSummaryItem();
            //item.FieldName = view.Columns[colIndex].FieldName;
            //item.ShowInGroupColumnFooter = view.Columns[colIndex];
            ...
            //item.DisplayFormat = view.Columns[colIndex].DisplayFormat.FormatString;
            //view.GroupSummary.Add(item);
            GridColumnSummaryItem item1 = new GridColumnSummaryItem(DevExpress.Data.SummaryItemType.Sum, view.Columns[colIndex].FieldName, "{0:n0}");
            view.Columns[view.Columns[colIndex].FieldName].Summary.Add(item1);
        }
        ...
        public void GridLayoutInit()
        {
            ...
            ModelService.Dispose();
            Handler.MessageBoxHandler.Show("UI가 초기화 되었습니다. 화면을 닫은 후 재실행 바랍니다.", "완료", MessageBoxButtons.OK, MessageBoxIcon.Information);
