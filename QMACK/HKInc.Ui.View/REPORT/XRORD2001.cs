    ...
    public partial class XRORD2001 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XRORD2001(string cust, string prtno) : this()
        {
            ...
            if (ds.Tables[0] != null)
            {
                ...
                tx_cust.Text = ds.Tables[0].Rows[0][1].ToString();
                ts_cust.Text = ds.Tables[0].Rows[0][1].ToString();
            }
            ...
            for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    labelval[i][j].Text = ds.Tables[1].Rows[i][j].ToString();
                    labelval1[i][j].Text = ds.Tables[1].Rows[i][j].ToString();
                }

                if (ds.Tables[1].Rows[i][6].ToString() != "")
                    Total_AMT += Convert.ToInt32(ds.Tables[1].Rows[i][6].ToString().Replace(",", ""));

                if (ds.Tables[1].Rows[i][7].ToString() != "")
                    Total_TAX += Convert.ToInt32(ds.Tables[1].Rows[i][7].ToString().Replace(",", ""));

            }

            tx_tax.Text = Total_TAX.ToString("0,0");
            tx_amt.Text = Total_AMT.ToString("0,0");
            ts_tax.Text = Total_TAX.ToString("0,0");
            ts_amt.Text = Total_AMT.ToString("0,0");

            tx_sumamt.Text = (Total_AMT + Total_TAX).ToString("0,0");
            ts_sumamt.Text = (Total_AMT + Total_TAX).ToString("0,0");
