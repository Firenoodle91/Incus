...
namespace HKInc.Ui.View.POP_Popup1
{
    public partial class XPFITEMMOVE : XtraForm
    {
        ...
        {
          POP_Popup.XRITEMMOVE prt;
            if (lpseq != 1) return;
                if (TX_NO.Text != "")
                {
                    prt = new POP_Popup.XRITEMMOVE("", "", TX_NO.Text);

                }
                else
                {
                    prt = new POP_Popup.XRITEMMOVE(lworkno, llotno, "");
               }
        ...
        private void XPFITEMMOVE_Load(object sender, EventArgs e)
        {
            if (lpseq != 1)
            {
                ...
                string tmoveno = DbRequestHandler.GetCellValue(sql, 0).GetNullToEmpty();
                if (tmoveno != "")
                {
                    POP_Popup.XRITEMMOVE prt = new POP_Popup.XRITEMMOVE("", "", tmoveno);
                    ReportPrintTool printTool = new ReportPrintTool(prt);
                    printTool.ShowPreviewDialog();
                    this.Close();
                }
