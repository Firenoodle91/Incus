    ...
    public partial class FPOP010 : HKInc.Service.Base.ListMasterDetailFormTemplate
    {
      ...
      //      MasterGridExControl.MainGrid.AddColumn("ItemCode", "품목코드");
        ...
        protected override void DataLoad()
        {
            ...
            MasterGridBindingSource.DataSource = ModelService.GetChildList<VI_RESULT_QTY>
                (p => p.ResultDate.Year == DateTime.Today.Year
                && (p.ResultDate.Month == DateTime.Today.Month)
                && (p.ResultDate.Day == DateTime.Today.Day)
                && p.ProcessCode == "P01").OrderBy(o => o.WorkNo).OrderBy(s => s.Pseq).ToList();
            MasterGridExControl.DataSource = MasterGridBindingSource;

            gx3bindingSource.DataSource = ModelService.GetChildList<VI_RESULT_QTY>
                (p => p.ResultDate.Year == DateTime.Today.Year
                && (p.ResultDate.Month == DateTime.Today.Month)
                && (p.ResultDate.Day == DateTime.Today.Day)
                && p.ProcessCode == "P03").OrderBy(o => o.WorkNo)
                .OrderBy(s => s.Pseq).ToList();
            gridEx3.DataSource = gx3bindingSource;

            gx4bindingSource.DataSource = ModelService.GetChildList<VI_RESULT_QTY>
                (p => p.ResultDate.Year == DateTime.Today.Year
                && (p.ResultDate.Month == DateTime.Today.Month)
                && (p.ResultDate.Day == DateTime.Today.Day)
                && p.ProcessCode == "P04").OrderBy(o => o.WorkNo).OrderBy(s => s.Pseq).ToList();
            gridEx4.DataSource = gx4bindingSource;

            gx5bindingSource.DataSource = ModelService.GetChildList<VI_RESULT_QTY>           
                (p => p.ResultDate.Year == DateTime.Today.Year
                && (p.ResultDate.Month == DateTime.Today.Month)
                && (p.ResultDate.Day == DateTime.Today.Day)
                && p.ProcessCode == "P06").OrderBy(o => o.WorkNo).OrderBy(s => s.Pseq).ToList();
            ...
            string memo = DbRequestHandler.GetCellValue("select  memo from Tn_memo", 0).GetNullToEmpty();
