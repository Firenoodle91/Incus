    ...
    public partial class XPFWORKEND : Service.Base.PopupCallbackFormTemplate
    {
        ...
        private void Btn_WorkEnd_Click(object sender, EventArgs e)
        {
            ...
            if (preProcessObj != null)
            {
                ...
                {
                    sMessage = "이전 공정에 대하여 작업이 완료되어 있지 않습니다. 확인 부탁드립니다.";
            ...
            {
                sMessage = "지시수량보다 총 생산량이 부족합니다. 무시하고 종료하시겠습니까?";
                if (MessageBoxHandler.Show(sMessage, "확인", MessageBoxButtons.YesNo) == DialogResult.No)
                    return;
            }

            sMessage = "정말로 종료하시겠습니까? ";

            if (MessageBoxHandler.Show(sMessage, "확인", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
