...
namespace HKInc.Ui.View.POP_Popup
{
    public partial class XPFSRCIN : XtraForm
    {
        ...
        private void SrcInfo()
        {
            ...
            DataTable dt = DbRequestHandler.GetDTselect("exec sp_srcin '" + tx_srcin.Text + "'");
            if (dt == null || dt.Rows.Count==0) {
                MessageBox.Show("원소재 품목코드가 잘목되었습니다.");
                tx_srcname.Text = "";
                tx_srcqty.Text = "";
                lbitem.Text = "";
                lb_lotno.Text = "";
            ...
            {
                string outf = DbRequestHandler.GetCellValue("exec SP_STOPINOUT '" + tx_srcin.Text + "'", 0);
                if (outf == "Y")
                {
