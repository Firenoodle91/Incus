...
namespace HKInc.Ui.View.POP_Popup
{
    public partial class XRITEMMOVE : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XRITEMMOVE(string workno,string lotno,string itemmoveno) : this()
        {
        ...
        {
            DataSet ds = DbRequestHandler.GetDataQury("exec SP_ITEM_MOVE_PRT @workno ='" + workno + "', @lotno='" + lotno + "'");// @moveno")
            if (ds == null) return;
            if (ds.Tables.Count != 2) return;
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                bar_moveno.Text = DbRequestHandler.GetRequestNumber("IMV");
            ...
            int k = DbRequestHandler.SetDataQury("insert into TN_ITEM_MOVE (ITEMMOVENO,WORKNO,LOTNO,QTY) values('" + bar_moveno.Text + "','" + workno + "','" + lotno + "' ,'')");
        }
        
        private void initdataR(string obj)
        {
            DataSet ds = DbRequestHandler.GetDataQury("exec SP_ITEM_MOVE_PRT  @moveno='"+ obj + "'");
            if (ds == null) return;
            if (ds.Tables.Count != 2) return;
            foreach (DataRow row in ds.Tables[0].Rows)
            {
