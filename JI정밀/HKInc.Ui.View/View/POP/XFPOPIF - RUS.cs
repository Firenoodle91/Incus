...
{
    public partial class XFPOPIF_RUS : HKInc.Service.Base.ListFormTemplate
    {
        ...
        public XFPOPIF_RUS()
        {
            ...
            try
            {
                ...
                hkrS232_RUS1.lsACnt = ini.IniReadValue("Serial", "Acnt", aa).GetIntNullToZero();
                hkrS232_RUS1.lsBCnt = ini.IniReadValue("Serial", "Bcnt", aa).GetIntNullToZero();
                HKInc.Service.Controls.HKRS232_RUS.lsPort = ini.IniReadValue("Serial", "port", aa).GetNullToEmpty();
                HKInc.Service.Controls.HKRS232_RUS.lsmc = ini.IniReadValue("Serial", "mc", aa).GetNullToEmpty();
            }
            catch
            {
                POP_POPUP.XFPRSSET_RUS f = new POP_POPUP.XFPRSSET_RUS();
        ...
        {
            if (simpleButton1.Text != "Начните работу") return;
            POP_POPUP.XFPRSSET_RUS f = new POP_POPUP.XFPRSSET_RUS();
            if (DialogResult.OK == f.ShowDialog())
            {
                lup_Machine.EditValue = ini.IniReadValue("Serial", "mc", aa).GetNullToEmpty();
        ...
        private void Btn_QualityAdd_Click(object sender, EventArgs e)
        {
            ...
            if (llot == "") { MessageBox.Show("LOT не создан.");return; }     // LOT가 생성되지 않았습니다.
            ...
            var inspectionForm = new POP_POPUP.XPFQCIN_RUS(workno, process, item,  "",tx_ProductLotNo.EditValue.GetNullToEmpty(),"IF"); //JI버전
        ...
        {
            if (simpleButton1.Text == "Начните работу")     // 시작
            {
                if (tx_workno.EditValue.GetNullToEmpty() == "") { MessageBox.Show("Невозможно запустить без указания задачи."); return; }        // 작업지시 없이 시작할수 없습니다.
                hkrS232_RUS1.Open();
                if (hkrS232_RUS1.lsportstate == "Open")
                {
                    simpleButton1.ImageOptions.Image = IconImageList.GetIconImage("arrows/pause");
                    simpleButton1.Text = "прекраще́ние";     // 중지
        ...
        private void dataload()
        {
            ...
            if (item == "") { MessageBox.Show("Предметы являются обязательными."); return; }        // 품목은 필수조건입니다.
            if (mc == "") { MessageBox.Show("Оборудование является обязательным условием."); return; }      // 설비는 필수조건입니다.
            if (process == "") { MessageBox.Show("Процесс является обязательным."); return; }     // 공정은 필수조건입니다.
            ...
            if (ds== null) { MessageBox.Show("Нет соответствующих инструкций по работе."); return; }     // 해당 작업지시가 없습니다.
            if (ds.Tables[0].Rows.Count == 0) { MessageBox.Show("Нет соответствующих инструкций по работе."); return; }     // 해당 작업지시가 없습니다.
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            decimal? qty = hkrS232_RUS1.lsACnt.GetIntNullToZero();
            if (qty != 0)
            {
                hkrS232_RUS1.lsACnt = 0;
        ...
        {
            POP_POPUP.XPFANDON_RUS f = new POP_POPUP.XPFANDON_RUS(tx_workno.EditValue.GetNullToEmpty(),
