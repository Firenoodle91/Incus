...
namespace HKInc.Ui.View.SELECT_POPUP
{
    ...
    public partial class XSFSTD1100 : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        ...
        protected override void DataLoad()
        {
            ...
            else if (Constraint == MasterCodeSTR.Contraint_ItemMAT_BAN)  //자재&반제품만조회, 임시로 완제품제외 모두 표시
            {
                bindingSource.DataSource = ModelService.GetList(p => (string.IsNullOrEmpty(itemCodeName) ? true : (p.ItemCode.Contains(itemCodeName) || (p.ItemName.Contains(itemCodeName) || p.ItemNameENG.Contains(itemCodeName) || p.ItemNameCHN.Contains(itemCodeName))))
                                                                        && (p.UseFlag == "Y")&&p.TopCategory!=MasterCodeSTR.TopCategory_WAN
                                                                        //&& (p.TopCategory == MasterCodeSTR.TopCategory_MAT || p.TopCategory == MasterCodeSTR.TopCategory_BAN)
                                                                        && (string.IsNullOrEmpty(CustomerCode) ? true : (p.MainCustomerCode == CustomerCode))
                                                               )
                                                               .OrderBy(p => p.ItemName)
                                                               .ToList();
        ...
        protected override void Confirm()
        {
            ...
            DateTime searchSt = Convert.ToDateTime(DateTime.Now);
            DateTime searchEt = Convert.ToDateTime("2999-12-31");
            ...
            if (IsmultiSelect)
            {
                ...
                {
                    // 단가이력정보 확인
                    var costList = ModelService.GetChildList<TN_STD1105>(w => w.ItemCode == v.ItemCode && w.StartDate <= searchSt && w.EndDate >= searchEt).FirstOrDefault(); 
                    var returnObj = ModelService.GetList(p => p.ItemCode == v.ItemCode).FirstOrDefault();

                    // 1순위 단가이력정보 단가 
                    // 2순위 품목에 등록된 단가
                    // 3순위 단가이력 null && 품목단가가 null 이면 0으로 표시한다
                    if (costList == null)
                    {
                        if (returnObj.Cost == null) returnObj.Cost = 0;
                    }
                    else if (costList != null)
                        returnObj.Cost = costList.ItemCost;

                    else
                        returnObj.Cost = 0;

                    //if (returnObj.Cost == null)
                    //{
                    //    //var costList = ModelService.GetChildList<TN_STD1105>(w => w.ItemCode == v.ItemCode && w.StartDate <= Convert.ToDateTime(DateTime.Now) && (w.EndDate >= Convert.ToDateTime(DateTime.Now) || w.EndDate >= Convert.ToDateTime("2999-12-31"))).FirstOrDefault();
                    //    var costList = ModelService.GetChildList<TN_STD1105>(w => w.ItemCode == v.ItemCode && w.StartDate <= searchSt && w.EndDate >= searchEt).FirstOrDefault();

                    //    //var itemCostList = ModelService.GetChildList<TN_STD1105>().Where(w => w.ItemCode == v.ItemCode).FirstOrDefault();
                    //    //var fileterCode = Convert.ToDecimal(ModelService.GetChildList<TN_STD1105>(w => w.ItemCode == v.ItemCode && w.StartDate == DateTime.Now).FirstOrDefault());
                    //    //decimal dItemCode = Convert.ToDecimal(ModelService.GetChildList<TN_STD1105>(w => w.ItemCode == v.ItemCode && w.StartDate == DateTime.Now).FirstOrDefault());
                    //    if(returnObj.Cost == null) { returnObj.Cost = 0; }
                    //    else { returnObj.Cost = costList.ItemCost; }
                    //}
