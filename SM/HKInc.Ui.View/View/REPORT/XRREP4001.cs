...
namespace HKInc.Ui.View.View.REPORT
{
    ...
    /// 품목별 불량률화면
    /// </summary>
    public partial class XRREP4001 : HKInc.Service.Base.ListFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("BadRate", LabelConvert.GetLabelText("BadRate"), HorzAlignment.Far, FormatType.Numeric, "{0:#,###,###,##0}%");
        ...
        protected override void DataLoad()
        {
            ...
            using (var context = new HKInc.Ui.Model.Context.ProductionContext(HKInc.Utils.Common.ServerInfo.GetConnectString(HKInc.Utils.Common.GlobalVariable.ProductionDataBase)))
            {
                ...
                var result = context.Database
                      .SqlQuery<TEMP_XRREP4001>("SP_FAIL_RAT @type, @fromdt,@todt ,@item", lType, fdt, tdt, itemcode).ToList();
            ...
            List<TEMP_XRREP4001> ds = GridBindingSource.DataSource as List<TEMP_XRREP4001>;
            //타이틀설정
            ...
            //X축설정 (Series)
            Chart1.DataSource = ds.GroupBy(p => new { p.ProcessName, p.ItemName }).Select(p => new TEMP_XRREP4001
            {
                ProcessName = p.Key.ProcessName
                 ,
                ItemName = p.Key.ItemName
                 ,
                BadRate = p.Sum(c => c.OkQty).GetDecimalNullToZero() == 0 ? 0 : (p.Sum(c => c.BadQty) / p.Sum(c => c.OkQty).GetDecimalNullToZero()) * 100
            }).ToList();
