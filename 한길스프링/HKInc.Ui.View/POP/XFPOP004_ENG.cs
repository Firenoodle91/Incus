...
namespace HKInc.Ui.View.POP
{
    ...
    /// </summary>
    public partial class XFPOP004_ENG : HKInc.Service.Base.ListFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("JobStatus", "Work" + Environment.NewLine +"Status", HorzAlignment.Center, true);
            //GridExControl.MainGrid.AddColumn("WorkDate", "작업지시일", HorzAlignment.Center, true);
            GridExControl.MainGrid.AddColumn("DueDate", "Delivery" + Environment.NewLine + "Plan Date", HorzAlignment.Center, true);
            GridExControl.MainGrid.AddColumn("WorkNo", "Work Order No", HorzAlignment.Center, false);
            GridExControl.MainGrid.AddColumn("Process", "ProcessCode", false);
            GridExControl.MainGrid.AddColumn("ProcessName", "Process");
            GridExControl.MainGrid.AddColumn("MachineCode", "MachineCode",false);
            GridExControl.MainGrid.AddColumn("MachineName", "Machine");
            GridExControl.MainGrid.AddColumn("CustomerCode", "CustomerName");
            GridExControl.MainGrid.AddColumn("ItemCode", "ItemCode", false);
            GridExControl.MainGrid.AddColumn("ItemNm1", "ItemCode", false);
            GridExControl.MainGrid.AddColumn("ItemNm", "ItemName");
            GridExControl.MainGrid.AddColumn("PlanQty", "Work" + Environment.NewLine + "Order Qty", HorzAlignment.Far, FormatType.Numeric, "n0");
            GridExControl.MainGrid.AddColumn("StdPackQty", "Packing" + Environment.NewLine + "Box Qty", HorzAlignment.Far, FormatType.Numeric, "n0");
            GridExControl.MainGrid.AddColumn("Memo", "Memo", false);
            ...
            GridExControl.MainGrid.MainView.Appearance.HeaderPanel.Font = new Font(DefaultFont.FontFamily, 13f, FontStyle.Bold);
            ...
            GridExControl.MainGrid.MainView.Columns["PlanQty"].MinWidth = 120;
            GridExControl.MainGrid.MainView.Columns["PlanQty"].MaxWidth = 120;
        ...
        private void pe_domap_DoubleClick(object sender, EventArgs e)
        {
            ...
            POP_Popup.XPFPOPIMG fm = new POP_Popup.XPFPOPIMG("Product Picture", pe_domap.EditValue);
            fm.ShowDialog();
        }
        ...
        private void jobstart()
        {
            ...
            if (obj.PSeq == 1)
            {
                if (obj.JobStatus == ((int)MasterCodeEnum.POP_Status_Wait).ToString())
                {
                    ...
                    POP_Popup.XPFSRCIN3_ENG fm = new POP_Popup.XPFSRCIN3_ENG();
                    fm.TP_POPJOB = obj;
                    fm.ShowDialog();
            ...
            else
            {
                ...
                if (tx_lotno.EditValue.GetNullToEmpty() == "")
                {
                    ...
                    POP_Popup.XPFITEMMOVESCAN_ENG fm = new POP_Popup.XPFITEMMOVESCAN_ENG(obj);
                    fm.ShowDialog();
        ...
        {
            DialogResult req = MessageBox.Show("Do you want to exit?", "Warning", MessageBoxButtons.OKCancel);
        ...
        private void btn_srcchange_Click(object sender, EventArgs e)
        {
            ...
            if (fm.DialogResult == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (tx_lotno.Text != "")
                {
                    ...
                    {
                        DialogResult = MessageBox.Show("LOT NO is changed. Would you like to register your existing LOT NO Results?", "Notice", MessageBoxButtons.YesNo);
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("LOT NO is the same as the original raw material.");
                    }
                }
                else
                {
        ...
        private void btn_moveprt_Click(object sender, EventArgs e)
        {
            ...
            POP_Popup.XPFITEMMOVE_ENG fm = new POP_Popup.XPFITEMMOVE_ENG(tn.WorkNo, tx_lotno.Text,tn.PSeq);
            fm.ShowDialog();
        }
        ...
        private void btn_end_Click(object sender, EventArgs e)
        {
            ...
            POP_Popup.XPFJOBEND_ENG efm = new POP_Popup.XPFJOBEND_ENG();
            efm.ShowDialog();
            switch (efm.lstatus)
            {
                case "qtytostop":
                    ...
                    {
                        DialogResult = MessageBox.Show("Do you want to output the document?", "Notice", MessageBoxButtons.YesNo);
                        if (DialogResult == DialogResult.Yes)
                        {
                            prtitemmove(obj);
                        }
        ...
        {
            POP_Popup.XPFITEMMOVE_ENG fm = new POP_Popup.XPFITEMMOVE_ENG(obj.WorkNo, tx_lotno.Text, obj.PSeq);
        ...
        {
            POP_Popup.XPFQTYIN2_ENG fm = new POP_Popup.XPFQTYIN2_ENG(obj, tx_lotno.Text, GridExControl.MainGrid.MainView.GetFocusedRowCellDisplayText("Process").ToString());
        ...
        private void btn_stopin_Click(object sender, EventArgs e)
        {
            ...
            if (cnt == "0")
            {
                POP_Popup.XPFSTOP_ENG fm = new POP_Popup.XPFSTOP_ENG(obj.MachineCode);
                fm.ShowDialog();
                if (fm.DialogResult == DialogResult.OK)
                {
        ...
        private void btn_qcin_Click(object sender, EventArgs e)
        {
            TP_POPJOBLIST obj = GridBindingSource.Current as TP_POPJOBLIST;
            if (obj == null) return;
            POP_Popup.XPFQCIN_ENG qc = new POP_Popup.XPFQCIN_ENG(obj,tx_lotno.Text.GetNullToEmpty());
            qc.ShowDialog();
        }
        ...
        private void btn_itemmove_Click(object sender, EventArgs e)
        {
            ...
            if (fm.DialogResult == DialogResult.OK)
            {
                ...
                {
                    //DialogResult = MessageBox.Show("Notice", "LOTNO가 변경됩니다. 기존 LOTNO 실적을 등록하시겠습니까?", MessageBoxButtons.YesNo);
                    DialogResult = MessageBox.Show("LOT NO is changed. Would you like to register your existing LOT NO Results?", "Notice", MessageBoxButtons.YesNo);
                    if (DialogResult == DialogResult.Yes)
                    {
                        ProdQtyin();
