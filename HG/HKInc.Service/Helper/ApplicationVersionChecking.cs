...
namespace HKInc.Service.Helper
{
    public class ApplicationVersionChecking
    {
            ...
            if (ApplicationDeployment.IsNetworkDeployed)
            {
                ...
                if (info.UpdateAvailable)
                {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("이 응용 프로그램은 현재 업데이트에서 필수 업데이트를 감지했습니다. " +
                    ...
                    if (doUpdate)
                    {
                        try
                        {
                            ...
                            MessageBox.Show("응용 프로그램이 업그레이드되었으며 이제 다시 시작됩니다.");
                            Application.Restart();
                        }
                        catch (DeploymentDownloadException dde)
                        {
                            MessageBox.Show("최신 버전의 응용 프로그램을 설치할 수 없음. \n\n네트워크 연결을 확인하거나 나중에 다시 시도하십시오. Error: " + dde);
                            CheckFlag = false;
                            return;
                        }
                    }
        ...
        public static bool SetSystemDateTime(DateTime dtNew)
        {
            ...
            var ds = Service.DbRequesHandler.GetDataQury(sql);
