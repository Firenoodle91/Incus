...
namespace HKInc.Ui.View.POP_Popup
{
    public partial class XPFSTOP : XtraForm
    {
            ...
            public XPFSTOP(string obj)
        {
            ...
            machine = obj;
            DataSet ds = DbRequesHandler.GetDataQury("SELECT       [MCODE]      ,[CODE_NAME] FROM [TN_STD1000T] where CODE_MAIN='P003' and CODE_MID!='00'");
           ...
           // MessageBox.Show(j.ToString() + "_"+k.ToString());
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                btn[i].Font = new Font("맑은 고딕", 13f, FontStyle.Bold);
                switch (i % 3)
                {
                    case 0:
