...
namespace HKInc.Ui.View.POP
{
    ...
    public partial class XFPOP003 : HKInc.Service.Base.ListFormTemplate
    {
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.MainView.Columns["WorkDate"].MinWidth = 120;
            GridExControl.MainGrid.MainView.Columns["WorkDate"].MaxWidth = 120;
            ...
            GridExControl.MainGrid.MainView.Columns["ProcessName"].MinWidth = 120;
            GridExControl.MainGrid.MainView.Columns["ProcessName"].MaxWidth = 120;
            GridExControl.MainGrid.MainView.Columns["MachineName"].MinWidth = 90;
            GridExControl.MainGrid.MainView.Columns["MachineName"].MaxWidth = 90;
            ...
            try
            {
                ...
                using (var context = new HKInc.Ui.Model.Context.ProductionContext(HKInc.Utils.Common.ServerInfo.GetConnectString(HKInc.Utils.Common.GlobalVariable.ProductionDataBase)))
                {
                    ...
                    GridBindingSource.DataSource = result.Where(p=>p.ItemNm1.Contains(ItemCodeName) || p.ItemNm.Contains(ItemCodeName))
                        .OrderBy(p=>p.WorkDate).ThenBy(p=>p.DisplayOrder).ThenBy(o => o.PSeq).ToList();
                    if(result.Count == 0)
                    {
                        tx_lotno.Text = "";
                        tx_makeqty.Text = "0";
