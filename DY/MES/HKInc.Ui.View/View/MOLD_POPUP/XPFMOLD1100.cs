...
namespace HKInc.Ui.View.View.MOLD_POPUP
{
    public partial class XPFMOLD1100 : HKInc.Service.Base.ListEditFormTemplate
    {
        ...
        public XPFMOLD1100(PopupDataParam param, PopupCallback callback)
        {
            ...
            tx_MoldMCode.KeyUp += Tx_MoldMCode_KeyUp;
        ...
        /// <summary>
        /// 20211025 오세완 차장
        /// 금형코드 자유입력으로 변경되어 로직이 추가, 입력시 금형코드 중복 체크
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Tx_MoldMCode_KeyUp(object sender, KeyEventArgs e)
        {
            if(EditMode == PopupEditMode.New)
            {
                TextEdit tb = sender as TextEdit;
                string sMoldmcode = tb.EditValue.GetNullToEmpty();
                if(sMoldmcode != "")
                {
                    List<TN_MOLD1100> tempList = ModelService.GetList(p => p.MoldMCode == sMoldmcode).ToList();
                    if(tempList !=  null)
                        if(tempList.Count > 0)
                        {
                            MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_41), LabelConvert.GetLabelText("MoldMCode"), LabelConvert.GetLabelText("MoldMCode")));
                            tb.EditValue = "";
                        }
                }
            }
        ...
        {
            // 20211025 오세완 차장 자유 코드로 변경해 달라고 하여 수정처리
            if (EditMode != PopupEditMode.New)
            ...
            // 20211025 오세완 차장 창고 코드를 추가했는데 정렬이 안되어서 수정
            List<TN_WMS2000> tempList = ModelService.GetChildList<TN_WMS2000>(p => p.UseFlag == "Y").ToList();
            tempList = tempList.OrderBy(o => o.Seq).ToList();
            lup_MoldWhPosition.SetDefault(true, "PositionCode", DataConvert.GetCultureDataFieldName("PositionName"), tempList);
        ...
        protected override void DataSave()
        {
            ...
            bool bCheckSave = false;
            string sMessage = "";
            if (EditMode == PopupEditMode.New)
            {
                // 20211025 오세완 차장 code 자유채번 때문에 추가된 검증 로직
                if(tx_MoldMCode.EditValue.GetNullToEmpty() == "")
                    MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_40), LabelConvert.GetLabelText("MoldMCode")));
                else if(tx_MoldMCode.EditValue.GetNullToEmpty().Length > 50)
                    MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_40), LabelConvert.GetLabelText("MoldMCode")));
                else
                {
                    string sMoldmcode = ""; 
                    if (obj.MoldMCode.Length > 10)
                        sMessage = "자릿수는 10자리를 넘을 수 없습니다.";
                    else
                    {
                        if(obj.MoldMCode.Length > 5)
                        {
                            sMoldmcode = obj.MoldMCode.Substring(0, 5);
                            if (sMoldmcode == "MOLD-")
                            {
                                int iTempseq;
                                string sMold_seq = obj.MoldMCode.Substring(5, obj.MoldMCode.Length - 5);
                                bool bConvert = int.TryParse(sMold_seq, out iTempseq);
                                if (bConvert)
                                {
                                    if (iTempseq < 0)
                                    {
                                        // 20211025 오세완 차장 mold--0001같은 것을 방지처리
                                        sMessage = "기준 채번은 MOLD-XXXXX형태입니다.";
                                    }
                                    else
                                    {
                                        if (sMold_seq.IndexOf("+") > -1)
                                        {
                                            // 20211025 오세완 차장 mold-+0001같은 것을 방지처리
                                            sMessage = "기준 채번은 MOLD-XXXXX형태입니다.";
                                        }
                                        ...
                                        {
                                            string sSql = "USP_INS_ITEM_SEQ 'MOLD'," + iTempseq.ToString() + "";
