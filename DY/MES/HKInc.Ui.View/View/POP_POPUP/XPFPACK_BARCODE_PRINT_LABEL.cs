...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    /// 포장라벨출력/박스라벨출력(바코드라벨)
    /// </summary>
    public partial class XPFPACK_BARCODE_PRINT_LABEL : Service.Base.PopupCallbackFormTemplate
    {
        TEMP_XFPOP_PACK TEMP_XFPOP_PACK;
        VI_MPS1800_LIST VI_MPS1800_LIST;
        string division = string.Empty; // 1 : 포장라벨출력 , 2 : 박스라벨출력
        public XPFPACK_BARCODE_PRINT_LABEL()
        {
            InitializeComponent();
        }

        public XPFPACK_BARCODE_PRINT_LABEL(PopupDataParam parameter, PopupCallback callback) : this()
        {
        ...
        protected override void DataPrint()
        {
            try
            {
                ...
                var mainReport = new REPORT.XRPACK_V2();
                if (printQty <= 1)
                {
                    //var report = new REPORT.XRPACK(TEMP_XFPOP_PACK, perBoxQty, DateTime.Today.ToShortDateString());
                    // 20210819 오세완 차장 대영 요청으로 10*6 크기로 변경
                    var report = new REPORT.XRPACK_V2(TEMP_XFPOP_PACK, perBoxQty, DateTime.Today.ToShortDateString());
