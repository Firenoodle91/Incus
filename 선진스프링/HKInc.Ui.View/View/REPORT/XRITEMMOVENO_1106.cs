...
namespace HKInc.Ui.View.View.REPORT
{
    ...
    /// 이동표 출력 양식
    /// </summary>
    public partial class XRITEMMOVENO_1106 : DevExpress.XtraReports.UI.XtraReport
    {
        XRLabel[][] lblList; 

        public XRITEMMOVENO_1106()
        {
            InitializeComponent();
        }

        public XRITEMMOVENO_1106(TEMP_ITEM_MOVE_NO_MASTER masterObj, List<TEMP_ITEM_MOVE_NO_DETAIL> detailList) : this()
        {
            ...
            var TN_STD1100 = ModelService.GetChildList<TN_STD1100>(p => p.ItemCode == masterObj.ItemCode).FirstOrDefault();
            ...
            lbl_SrcItemCode.Text = TN_STD1100 == null ? string.Empty : TN_STD1100.SrcCode;
            lbl_BoxInQty.Text = masterObj.BoxInQty.GetDecimalNullToZero().ToString("#,0.##");
            lbl_PrintDate.Text = masterObj.PrintDate.ToString("yyyy-MM-dd");

            var sumObj = ModelService.GetChildList<TN_MPS1201>(p => p.WorkNo == masterObj.WorkNo && p.ProcessSeq == 1).ToList();
            lbl_SumResultQty.Text = sumObj.Count == 0 ? "0" : sumObj.Sum(p => p.ResultSumQty).GetDecimalNullToZero().ToString("#,0.##");
