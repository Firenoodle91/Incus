...
namespace HKInc.Ui.View.View.POP_POPUP
{
    ...
    public partial class XPFMACHINECHECK : Service.Base.ListFormTemplate
    {
        ...
        public XPFMACHINECHECK(string procTeamCode)
        {
            InitializeComponent();
        ...
        protected override void InitCombo()
        {
            ...
            var checkMachineArray = ModelService.GetChildList<TN_MEA1003>(p => p.CheckDate == DateTime.Today).Select(p => p.MachineCode).Distinct().ToArray();
            lup_Machine.SetDefaultPOP(false, "MachineMCode", DataConvert.GetCultureDataFieldName("MachineName"),
                ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y"
                                                        && p.UseFlag == "Y"
                                                        && (string.IsNullOrEmpty(procTeamCode) ? true : p.ProcTeamCode == procTeamCode)
                                                        && !checkMachineArray.Contains(p.MachineMCode)
            ...
            //if (userObj == null) 
            //    lup_Machine.SetDefaultPOP(false, "MachineMCode", DataConvert.GetCultureDataFieldName("MachineName"), ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseFlag == "Y").ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
            //else
            //    lup_Machine.SetDefaultPOP(false, "MachineMCode", DataConvert.GetCultureDataFieldName("MachineName"), ModelService.GetChildList<TN_MEA1000>(p => p.DailyCheckFlag == "Y" && p.UseFlag == "Y" && p.ProcTeamCode == userObj.ProductTeamCode).ToList(), DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor);
