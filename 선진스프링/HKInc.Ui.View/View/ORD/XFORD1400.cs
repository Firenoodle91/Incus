...
namespace HKInc.Ui.View.View.ORD
{
    ...
    public partial class XFORD1400 : HKInc.Service.Base.ListMasterDetailFormTemplate
    {
        ...
        private void MainView_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            ...
            else if (e.Column.FieldName == "OutLotNo")
            {
                if (!detailObj.OutLotNo.IsNullOrEmpty())
                {
                    if (ModelService.GetChildList<TN_ORD1401>(p => p.OutLotNo == detailObj.OutLotNo && p.TN_ORD1400.ItemCode != obj.ItemCode).Count > 0)
                    {
                        MessageBoxHandler.Show("해당 LOT NO는 다른 품목으로 이미 등록되어 있기 때문에 사용하실 수 없습니다.");
                        detailObj.OutLotNo = null;
                    }
                }
            }
        }
        ...
        private void MainView_ShowingEditor(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (view.FocusedColumn.FieldName == "OutLotNo")
            {
                TN_ORD1400 obj = MasterGridBindingSource.Current as TN_ORD1400;
