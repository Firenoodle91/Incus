    ...
    public partial class XFORD1500 : Service.Base.SixGridFormTemplate
    {
        ...
        protected override void OneGridAddRowClicked()
        {
            ...
            else
            {
                ...
                var deadLineDate = lastObj == null ? todayPreviousMonth.AddDays(1 - today.Day) : lastObj.DeadLineDate.AddMonths(1);
                if (lastObj != null)
                {
                    ...
                    if (today.Year <= deadLineDate.Year && today.Month <= deadLineDate.Month) //마감될 항목이 당월보다 낮지 않을 경우 예외처리 (당월 마감은 익월 가능)
                    {
        ...
        protected override void FiveGridAddRowClicked()
        {
            ...
            else
            {
                ...
                //if (today.Day < 20)
                //{
                //    //20일 미만이면 ex 8월 17일이면 8월달 마감 불가, 7월달까지 가능
                //    var checkDate = DateTime.Today.AddMonths(-1);

                //}
                //else
                //{
                //    //20일 이상이면 ex 8월 20일이면 8월달까지 마감 가능
                ...
                {
                    //20일 이상이면 다음달 재고
                    if (today.Day >= 20 && today.AddMonths(1).Year <= deadLineDate.Year && today.AddMonths(1).Month <= deadLineDate.Month)
                    {
                        MessageBoxHandler.Show(today.AddMonths(1).ToString("yyyy-MM") + " 마감은 " + today.AddMonths(1).ToString("yyyy-MM-") + "20 부터 가능합니다.");
                        return;
                    }

                    //if (today.Year <= deadLineDate.Year && )
                    //if (today.Day < 20 && today.M)
                    //{
                    ...
                    //if (today.Year <= deadLineDate.Year && today.Month <= deadLineDate.Month) //마감될 항목이 당월보다 낮지 않을 경우 예외처리 (당월 마감은 익월 가능)
                    //{
                    //    MessageBoxHandler.Show(today.ToString("yyyy-MM") + " 마감은 " + today.ToString("yyyy-MM-") + "20 부터 가능합니다.");
                    //    //MessageBoxHandler.Show(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_82));
                    //    return;
                    //}
                }
