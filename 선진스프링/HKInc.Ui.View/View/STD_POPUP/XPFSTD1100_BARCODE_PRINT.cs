...
namespace HKInc.Ui.View.View.STD_POPUP
{
    ...
    /// 품목 기타라벨출력
    /// </summary>
    public partial class XPFSTD1100_BARCODE_PRINT : Service.Base.PopupCallbackFormTemplate
    {
        ...
        public XPFSTD1100_BARCODE_PRINT()
        {
            InitializeComponent();
        }

        public XPFSTD1100_BARCODE_PRINT(PopupDataParam parameter, PopupCallback callback) : this()
        {
        ...
        protected override void InitCombo()
        {
            ...
            if (division == "4")
            {
                cbo_PrintDivision.ReadOnly = true;
                spin_PerBoxQty.EditValue = TN_STD1100_Obj.PackQty.GetDecimalNullToZero();
                dt_PrintDate.DateTime = DateTime.Today;

                var TN_STD1100 = TN_STD1100_Obj;
                tx_ItemCode.EditValue = TN_STD1100.ItemCode;
                tx_ItemName.EditValue = TN_STD1100.ItemName;
                tx_CustomerName.EditValue = "(주)선진스프링";
        ...
        protected override void DataPrint()
        {
            try
            {
                ...
                if (division == "4")
                {
                    if (TN_STD1100_Obj == null || printQty == 0) return;
                    var itemCode = tx_ItemCode.EditValue.GetNullToEmpty();
                    var lotNo = tx_LotNo.EditValue.GetNullToEmpty();
                    var customerName = tx_CustomerName.EditValue.GetNullToEmpty();
                    var mainReport = new REPORT.XRPACK_DELIV();
                    if (printQty < 11)
                    {
                        if (!firstFlag)
                        {
                            mainReport.SetBinding(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, printQty,customerName, printDate);
                            mainReport.CreateDocument();
                            firstFlag = true;
                        }
                        else
                        {
                            var report = new REPORT.XRPACK_DELIV(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, printQty, customerName, printDate);
                    ...
                    else
                    {
                        ...
                        for (int i = 1; i <= valueCount; i++)
                        {
                            if (!firstFlag)
                            {
                                mainReport.SetBinding(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, 10, customerName, printDate);
                                mainReport.CreateDocument();
                                firstFlag = true;
                            }
                            else
                            {
                                var report = new REPORT.XRPACK_DELIV(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, 10, customerName, printDate);
                                report.CreateDocument();
                                mainReport.Pages.AddRange(report.Pages);
                            }
                        }
                        ...
                        if (modCount != 0)
                        {
                            if (!firstFlag)
                            {
                                mainReport.SetBinding(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, modCount, customerName, printDate);
                                mainReport.CreateDocument();
                                firstFlag = true;
                            }
                            else
                            {
                                var report = new REPORT.XRPACK_DELIV(TN_STD1100_Obj, itemCode, itemName, lotNo, perBoxQty, modCount, customerName, printDate);
                                report.CreateDocument();
                                mainReport.Pages.AddRange(report.Pages);
                            }
                        }
