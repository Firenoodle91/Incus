...
namespace HKInc.Ui.View.SELECT_POPUP
{
    ...
    /// 제품 재고 Select 팝업
    /// </summary>
    public partial class XSFBANPROD_STOCK : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        IService<VI_BAN_STOCK_PRODUCT_LOT_NO> ModelService = (IService<VI_BAN_STOCK_PRODUCT_LOT_NO>)ProductionFactory.GetDomainService("VI_BAN_STOCK_PRODUCT_LOT_NO");
        BindingSource bindingSource = new BindingSource();
        private bool IsmultiSelect = true;
        private string Constraint = string.Empty;

        public XSFBANPROD_STOCK()
        {
            InitializeComponent();
        }

        public XSFBANPROD_STOCK(PopupDataParam parameter, PopupCallback callback) : this()
        {
        ...
        protected override void Confirm()
        {
            ...
            if (IsmultiSelect)
            {
                var returnList = new List<VI_BAN_STOCK_PRODUCT_LOT_NO>();
                var dataList = bindingSource.List as List<VI_BAN_STOCK_PRODUCT_LOT_NO>;
            ...
            {
                param.SetValue(PopupParameter.ReturnObject, ModelService.Detached((VI_BAN_STOCK_PRODUCT_LOT_NO)bindingSource.Current));
            }
        ...
        private void MainView_RowClick(object sender, RowClickEventArgs e)
        {
            ...
            if (e.Clicks == 2)
            {
                var returnObj = (VI_BAN_STOCK_PRODUCT_LOT_NO)bindingSource.Current;
                if (IsmultiSelect)
                {
                    var returnList = new List<VI_BAN_STOCK_PRODUCT_LOT_NO>();
                    if (returnObj != null)
