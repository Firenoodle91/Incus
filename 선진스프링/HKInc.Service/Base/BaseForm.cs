...
namespace HKInc.Service.Base
{
    ...
    public partial class BaseForm : XtraForm, IBaseForm, IFormControlChanged
    {
        ...
        void SetToolBar()
        {
            ButtonList.Add(ToolbarButton.Refresh, new BarButton() { ToolbarButton = barButtonRefresh, ClickAction = ActRefresh, ToolbarButtonCaption = LabelConvert.GetLabelText("Refresh") });
            ButtonList.Add(ToolbarButton.Save, new BarButton() { ToolbarButton = barButtonSave, ClickAction = ActSave, ToolbarButtonCaption = LabelConvert.GetLabelText("Save") });
            ButtonList.Add(ToolbarButton.Export, new BarButton() { ToolbarButton = barButtonExport, ClickAction = DataExport, ToolbarButtonCaption = LabelConvert.GetLabelText("Export") });
            ButtonList.Add(ToolbarButton.Print, new BarButton() { ToolbarButton = barButtonPrint, ClickAction = DataPrint, ToolbarButtonCaption = LabelConvert.GetLabelText("Print") });
            ButtonList.Add(ToolbarButton.Close, new BarButton() { ToolbarButton = barButtonClose, ClickAction = ActClose, ToolbarButtonCaption = LabelConvert.GetLabelText("Close") });
            ButtonList.Add(ToolbarButton.Confirm, new BarButton() { ToolbarButton = barButtonConfirm, ClickAction = ActConfirm, ToolbarButtonCaption = LabelConvert.GetLabelText("Confirm") });
        ...
        protected void ActSave()
        {
            ...
            catch (DbEntityValidationException ex)
            {
                ...
                {
                    dispalyMessage += string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_21)
                                                , LabelConvert.GetLabelText(eve.Entry.Entity.GetType().Name)
                                                , LabelConvert.GetLabelText(eve.Entry.State.GetNullToEmpty())
                                                , Environment.NewLine);

                    foreach (var ve in eve.ValidationErrors)                    
                        dispalyMessage += string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_20)
                                                        , ve.PropertyName
                                                        , string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_40), LabelConvert.GetLabelText(ve.ErrorMessage))
                                                        , Environment.NewLine);                    
                }
        ...
        public Bar GetBarTools
        {
            get { return this.barTools; }
        }
        ...
        public void SetToolbarVisible(bool visible)
        {
            ...
            if (!visible)
            {
                SetToolbarButtonVisible(visible);
            }
        }
        ...
        public void SetToolbarButtonVisible(ToolbarButton button, bool visible)
        {
            ...
            ButtonList[button].ToolbarButton.Enabled = visible;
        }
        ...
        {
            IsFormControlChanged = false;
            //카운트 미사용(gridView에서 이미 하단에 지원)
            //SetMessage(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_2), rowCount));
        }

        protected void SetRefreshMessage(GridEx gridEx)
        {
            IsFormControlChanged = false;
            //카운트 미사용(gridView에서 이미 하단에 지원)
            //SetMessage(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_2), gridEx.MainGrid.MainView.RowCount));
        }
        ...
        {
            IsFormControlChanged = false;
            SetMessage(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_3));
        }
