...
namespace HKInc.Service.Handler
{
    class UserLoginHandler : IUserLogin
    {
        ...
        private string GetModeRegistry()
        {
            string mode = RegistryHandler.GetValue(GlobalVariable.ModePath, "MODE");
        ...
        private void SetModeToRegistry(string mode)
        {
            if (saveUserId)
                RegistryHandler.SetValue(GlobalVariable.ModePath, "MODE", mode);
            else
                RegistryHandler.SetValue(GlobalVariable.ModePath, "MODE", string.Empty);
        }
