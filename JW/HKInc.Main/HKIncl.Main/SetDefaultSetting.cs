    ...
    public static class DefaultSetting
    {
        public static void SetDefaultValue()
        {
            ...
            try
            {
                ...
                if (GlobalVariable.LocalIP != "Y")
                {
                    GlobalVariable.FTP_SERVER = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 0);
                    GlobalVariable.HTTP_SERVER = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 1);
                }
                else {
                    string[] ftp = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 0).Split(':');
                    string[] http = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 1).Split(':');
                    GlobalVariable.FTP_SERVER = ftp[0] + "://" + ServerInfo.Server + ":" + ftp[2];
                    GlobalVariable.HTTP_SERVER = http[0] + "://" + ServerInfo.Server + ":" + http[2];
            ...
            try
            {
                ...
                GlobalVariable.ALamST1 = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 0);
                GlobalVariable.ALamSM1= HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 1);
                GlobalVariable.ALamST2 = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 2);
                GlobalVariable.ALamSM2 = HKInc.Service.Service.DbRequesHandler.GetCellValue(sql, 3);
            }
            catch { }
