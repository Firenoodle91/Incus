...
{
    public partial class XRITEMMOVE_100X100 : DevExpress.XtraReports.UI.XtraReport
    {
        XRLabel[][] labelval; 
        public XRITEMMOVE_100X100()
        {
            InitializeComponent();
        }
        public XRITEMMOVE_100X100(string workno,string lotno,string itemmoveno)
        {
            InitializeComponent();
        ...
        private void initdata(string workno,string lotno)
        {
            ...
            {
                bar_moveno.Text = tx_moveno.Text= DbRequesHandler.GetRequestNumber("IMV");
        ...
        private void initdataR(string obj)
        {
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                bar_moveno.Text = tx_moveno.Text = obj;
                tx_workno.Text = row[0].ToString();
                tx_lotno.Text = row[1].ToString();
                tx_itemnm.Text = row[3].ToString();
                tx_custnm.Text = row[4].ToString();
