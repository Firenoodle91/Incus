...
namespace HKInc.Service.Controls
{
    ...
    public partial class SearchLookUpEditEx : SearchLookUpEdit
    {
        ...
        /// 검색가능 팝업 
        /// </summary>
        /// <param name="bDeleteButtonVisible"></param>
        ...
        private void ItemCodeColumnSetting(RepositoryItemSearchLookUpEdit edit, string fieldName, string valueMember, string displayMember)
        {
            ...
            if (displayMember == "ItemCode")
            {
                ...
                {
                    FieldName = "ItemNm",
                    //Caption = "품명", // 20220218 오세완 차장 품목기준정보 원소재에서 출력명이 잘못된 것을 수정 
                    Caption = "품목", 
                    Visible = true
                });
            }
            else
            {
                ...
                {
                    FieldName = displayMember,
                    Caption = "품목",                 // 2022-02-23 김진우 품번에서 품목으로 수정
                    Visible = true
                });

                edit.View.Columns.Add(new DevExpress.XtraGrid.Columns.GridColumn
                {
                    FieldName = "ItemNm1",              // 2022-02-23 김진우 ItemNm 에서 ItemNm1로 수정
                    Caption = "품번",                   // 2022-02-23 김진우 품명에서 품번으로 수정
                    Visible = true
                });
            }
        }
        ...
        public void SetHAlignment(HorzAlignment horz = HorzAlignment.Default)
        {
            ...
            // 20220313 오세완 차장 Dataload에서 initcombo를 재실행하기 위해 수정
                    ...
                    if(ipc.PopupWindow.Controls != null)
                        if(ipc.PopupWindow.Controls.Count > 2)
                            if(ipc.PopupWindow.Controls[2].Controls != null)
                                if(ipc.PopupWindow.Controls[2].Controls.Count >= 1) // 20211021 오세완 차장 xfpur1300에서 이부분이 null처리가 된다. 
