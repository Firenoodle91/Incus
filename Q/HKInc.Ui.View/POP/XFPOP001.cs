    ...
    public partial class XFPOP001 : HKInc.Service.Base.ListFormTemplate
    {
        ...
        private void inicombo()
        {
         //   luptem.SetDefault(true, "Mcode", "Codename", DbRequesHandler.GetCommCode(MasterCodeSTR.tem));
            ...
            //lup_Item.SetDefault(true, "ItemCode", "ItemNm", ModelService.GetChildList<TN_STD1100>(p => p.TopCategory != "P03" && p.UseYn == "Y").OrderBy(o => o.ItemNm1).ToList());
        ...
        protected override void InitGrid()
        {
            ...
            GridExControl.MainGrid.AddColumn("ItemCode1", "규격");
        ...
        protected override void InitRepository()
        {
            ...
            GridExControl.MainGrid.SetRepositoryItemSearchLookUpEdit("Process", DbRequestHandler.GetCommCode(MasterCodeSTR.Process), "Mcode", "Codename");
            ...
            GridExControl.MainGrid.SetRepositoryItemSearchLookUpEdit("ItemCode", ModelService.GetChildList<TN_STD1100>(p => p.UseYn == "Y" && p.TopCategory != "P03").OrderBy(o => o.ItemNm).ToList(), "ItemCode", "ItemNm");
        ...
        private void rowchange()
        {
            ...
            if (tn1401 != null)
            {
                ...
                string sql = "SELECT isnull(sum(isnull([OK_QTY],0)),0) OkQty  ,isnull(sum(isnull([RESULT_QTY],0)),0) ResultQty   ,isnull(sum(isnull([FAIL_QTY],0)),0) FailQty   "
                   + "  FROM[HKInc_Data_QMackM].[dbo].[TN_MPS1405T] where RESULT_DATE = '" + DateTime.Today.ToString("yyyy-MM-dd") + "' and work_no='"+obj.WorkNo+"' and process_code='" + obj.Process + "'";
            ...
            string sql1 = "SELECT isnull(sum(isnull([RESULT_QTY],0)),0) ResultQty   "
                + "  FROM[HKInc_Data_QMackM].[dbo].[TN_MPS1405T] where RESULT_DATE = '" + DateTime.Today.AddDays(-1).ToString("yyyy-MM-dd") + "' and work_no='" + obj.WorkNo + "' and process_code='" + obj.Process + "'";
            DataSet ds1 = DbRequestHandler.GetDataQury(sql1);
            tx_preqty.Text ="전일실적: "+  Convert.ToString(ds1.Tables[0].Rows[0][0].GetIntNullToZero());
            tx_imv.Text = DbRequestHandler.GetCellValue("SELECT TOP 1 [ITEMMOVENO]   FROM [TN_ITEM_MOVE] where WORKNO='" + obj.WorkNo + "' and LOTNO='" + tx_lotno.Text + "' ", 0).GetNullToEmpty();
            tx_totok.Text = DbRequestHandler.GetCellValue("SELECT sum([OK_QTY]) qty FROM TN_MPS1401T where WORK_NO='" + obj.WorkNo + "' and PROCESS_CODE='" + obj.Process + "' ", 0).GetNullToEmpty();
        ...
        private void jobstart()
        {
            ...
            if (obj.PSeq == 1)
            {
                if (obj.JobStatus == ((int)MasterCodeEnum.POP_Status_Wait).ToString())
                {
                    ...
                    {
                        DataSet ds1 = DbRequestHandler.GetDataQury("exec SP_LOTMAKE1 @workno='" + obj.WorkNo + "',@mccode='" + obj.MachineCode + "',@srccode='" + fm.returnval[0] + "',@item='" + obj.ItemCode + "',@date='" + DateTime.Today.ToString("yyyyMMdd") + "',@srclot='" + fm.returnval[1] + "'");
                        tx_lotno.Text = ds1.Tables[0].Rows[0][0].ToString();

                        int cnt = DbRequestHandler.GetRowCount("select count(*) from tn_mps1401t where work_no='" + obj.WorkNo + "' and process_code='" + obj.Process + "'");
                        TN_MPS1401 newobj = new TN_MPS1401();
                        // 20220427 오세완 차장 nullable조건 삭제로 변경
                        //newobj.WorkDate = obj.WorkDate;
                        newobj.WorkDate = Convert.ToDateTime(obj.WorkDate);
                ...
                {
                    DataSet ds1 = DbRequestHandler.GetDataQury("exec SP_LOTMAKE1 @workno='" + obj.WorkNo + "',@date='" + DateTime.Today.ToString("yyyyMMdd") + "'");
                    if (ds1!=null)
                    {

                        tx_lotno.Text = ds1.Tables[0].Rows[0][0].ToString();
                        int cnt1 = DbRequestHandler.GetRowCount("select count(*) From tn_mps1401t where work_no='" + obj.WorkNo + "' and process_code='" + obj.Process + "' and lot_no='" + tx_lotno.Text + "'");
                        int cnt = DbRequestHandler.GetRowCount("select count(*) from tn_mps1401t where work_no='" + obj.WorkNo + "' and process_code='" + obj.Process + "'");
                        if (cnt1 == 0)
                        {
                            TN_MPS1401 newobj = new TN_MPS1401();
                    ...
                    else
                    {
                        ...
                        if (fm.DialogResult == System.Windows.Forms.DialogResult.OK)
                        {
                            DataSet ds2 = DbRequestHandler.GetDataQury("exec SP_LOTMAKE1 @workno='" + obj.WorkNo + "',@mccode='" + obj.MachineCode + "',@srccode='" + fm.returnval[0] + "',@item='" + obj.ItemCode + "',@date='" + DateTime.Today.ToString("yyyyMMdd") + "',@srclot='" + fm.returnval[1] + "'");
        ...
        private void fload()
        {
            //사용안함 전일 실적조회
            TP_POPJOBLIST obj = GridBindingSource.Current as TP_POPJOBLIST;
            string lslotno = DbRequestHandler.GetCellValue("SELECT max([LOT_NO]) lotno FROM TN_LOT_MST where WORK_NO='" + obj.WorkNo + "'", 0);
            ModelService.ReLoad();
        ...
        private void btn_srcchange_Click(object sender, EventArgs e)
        {
            ...
            if (obj.PSeq == 1)
            {
                ...
                if (fm.DialogResult == System.Windows.Forms.DialogResult.OK)
                {
                    if (fm.returnval[1] != DbRequestHandler.GetCellValue("select SRC_LOT from tn_lot_mst where lot_no='" + tx_lotno.Text + "'", 0))
                    {

                        DataSet ds = DbRequestHandler.GetDataQury("exec SP_LOTMAKE1 @workno='" + obj.WorkNo + "',@mccode='" + obj.MachineCode + "',@srccode='" + fm.returnval[0] + "',@item='" + obj.ItemCode + "',@date='" + DateTime.Today.ToString("yyyyMMdd") + "',@srclot='" + fm.returnval[1] + "'");
