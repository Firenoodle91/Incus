...
{
    /// <summary>
    /// 대영 스타일 실적등록 팝업 (기본) 러시아
    /// 2022-08-12 김진우
    /// </summary>
    public partial class XPFRESULT_DEFAULT_RUS : Service.Base.PopupCallbackFormTemplate
    {
        #region 전역변수
        ...
        public XPFRESULT_DEFAULT_RUS()
        {
            InitializeComponent();
        }

        public XPFRESULT_DEFAULT_RUS(PopupDataParam parameter, PopupCallback callback) : this()
        {
            this.PopupParam = parameter;
            this.Callback = callback;
            //this.Text = LabelConvert.GetLabelText("ResultAdd");
            this.Text = "Оформление выполнения работ";      // 실적등록
        ...
        protected override void InitControls()
        {
            ...
            {
                MessageBoxHandler.Show("Производство LOTNO не существует.");        // 생산LOTNO가 존재하지 않습니다
                ActClose();
            }
            else
            {
                ...
                if (MasterObj == null)
                {
                    MessageBoxHandler.Show("Отсутствует производительность.");      // 작업실적이 존재하지 않습니다
                    ActClose();
                }
                else
                {
        ...
        private void Btn_Add_Click(object sender, EventArgs e)
        {
            ...
            if(sBadtype == "")
            {
                sMessage = "Нет дефектного типа.";      // 불량 유형이 없습니다
            ...
            {
                sMessage = "Нет дефектного количества.";     // 불량 수량이 없습니다
                MessageBoxHandler.Show(sMessage);
                return;
            }
        ...
        protected override void InitGrid()
        {
            ...
            gridEx1.MainGrid.MainView.Columns["BadType"].Caption = "вид несоответствия";            // 불량유형
            gridEx1.MainGrid.MainView.Columns["BadQty"].Caption = "количество плохого";              // 불량수량
        ...
        private void Btn_Apply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBoxHandler.Show("Оператор обязателен");          // 작업자는 필수입니다
                    return;
                }
                ...
                {
                    //불량 수량이 존재합니다. 불량 유형을 추가해 주시기 바랍니다.
                    MessageBoxHandler.Show("Недопустимое количество уже существует. Добавьте тип дефекта.");
                    return;
                }
                else
                    bType_null = true;
                ...
                if (badQty == 0 && badType != null)
                {
                    // 불량 유형이 존재합니다. 불량 수량을 추가해 주시기 바랍니다.
                    MessageBoxHandler.Show("Недопустимый тип существует. Добавьте количество дефектов.");
                    return;
                }
                else
                    bQty_zero = true;
                ...
                else
                {
                    ...
                    {
                        // 불량 유형을 추가해 주시기 바랍니다.
                        MessageBoxHandler.Show("Добавьте тип дефекта.");
                        return;
                    }
                }
                ...
                if (badQty == 0)
                {
                    ...
                    {
                        // 생산 실적이 입력되지 않았습니다.
                        MessageBoxHandler.Show("Производительность не указана");
                        return;
                    }
                }
                ...
                {
                    // 불량 실적이 생산 실적보다 클 수 없습니다.
                    MessageBoxHandler.Show("Плохая производительность не может быть больше, чем производственная");
                    return;
                }
                #endregion
