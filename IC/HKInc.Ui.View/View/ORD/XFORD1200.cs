        ...
        private void Edit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    else if(masterObj.ItemCode != stockObj.ItemCode)
                    {
                        // 20220207 오세완 차장 품목코드 비교 로직이 없어서 추가 처리
                        MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_171), LabelConvert.GetLabelText("ItemCode")));
                    }
                    else
                    {
                ...
                {
                    textEdit.EditValue = "";
                    //e.Handled = true; // 202202027 오세완 차장 없는게 처리가 더 잘되는 듯 해서 생략
                }
            }
        }
