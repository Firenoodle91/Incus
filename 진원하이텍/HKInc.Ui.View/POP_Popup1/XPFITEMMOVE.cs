...
namespace HKInc.Ui.View.POP_Popup1
{
    public partial class XPFITEMMOVE : XtraForm
    {
        ...
        {
          POP_Popup.XRITEMMOVE_100X100 prt;
            if (lpseq != 1) return;
                if (TX_NO.Text != "")
                {
                    prt = new POP_Popup.XRITEMMOVE_100X100("", "", TX_NO.Text);

                }
                else
                {
                    prt = new POP_Popup.XRITEMMOVE_100X100(lworkno, llotno, "");
               }
        ...
        private void XPFITEMMOVE_Load(object sender, EventArgs e)
        {
            if (lpseq != 1)
            {
                ...
                if (tmoveno != "")
                {
                    POP_Popup.XRITEMMOVE_100X100 prt = new POP_Popup.XRITEMMOVE_100X100("", "", tmoveno);
                    ReportPrintTool printTool = new ReportPrintTool(prt);
                    printTool.ShowPreviewDialog();
                    this.Close();
                }
