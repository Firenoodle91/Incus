...
namespace HKInc.Ui.View.SELECT_POPUP
{
    ...
    /// 공정검사 대상 Select 팝업
    /// </summary>
    public partial class XSFQCT_PROCESS : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        IService<VI_INSP_PROCESS_OBJECT> ModelService = (IService<VI_INSP_PROCESS_OBJECT>)ProductionFactory.GetDomainService("VI_INSP_PROCESS_OBJECT");
        private bool IsmultiSelect = true;

        public XSFQCT_PROCESS()
        {
            InitializeComponent();
        }

        public XSFQCT_PROCESS(PopupDataParam parameter, PopupCallback callback) : this()
        {
            this.PopupParam = parameter;
            this.Callback = callback;
            this.Text = LabelConvert.GetLabelText("Inspection_PROCESS_Object");
        ...
        protected override void Confirm()
        {
            ...
            //if (IsmultiSelect)
            //{
            //    var returnList = new List<VI_INSP_PROCESS_OBJECT>();
            //    var dataList = ModelBindingSource.List as List<VI_INSP_PROCESS_OBJECT>;
            ...
            //else
            //{
            //    param.SetValue(PopupParameter.ReturnObject, ModelService.Detached((VI_INSP_PROCESS_OBJECT)ModelBindingSource.Current));
            //}
            //ReturnPopupArgument = new PopupArgument(param);
            //ActClose();

            var obj = (VI_INSP_PROCESS_OBJECT)ModelBindingSource.Current;
            if (IsmultiSelect)
            {
                var returnList = new List<VI_INSP_PROCESS_OBJECT>();
                if (obj != null)
