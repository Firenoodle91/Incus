...
namespace HKInc.Ui.View.SELECT_POPUP
{
    ...
    /// 발주복사 팝업
    /// </summary>
    public partial class XSFPUR1100_COPY : HKInc.Service.Base.PopupCallbackFormTemplate
    {
        IService<TN_PUR1100> ModelService = (IService<TN_PUR1100>)ProductionFactory.GetDomainService("TN_PUR1100");
        private bool IsmultiSelect = true;

        public XSFPUR1100_COPY()
        {
            InitializeComponent();
        }

        public XSFPUR1100_COPY(PopupDataParam parameter, PopupCallback callback) : this()
        {
            this.PopupParam = parameter;
            this.Callback = callback;
            this.Text = LabelConvert.GetLabelText("PoCopy");
        ...
        protected override void DataLoad()
        {
            ...
            {
                MessageBoxHandler.Show(string.Format(MessageHelper.GetStandardMessage((int)StandardMessageEnum.M_40), LabelConvert.GetLabelText("ProdItem")));
                return;
            }

            //ModelBindingSource.DataSource = ModelService.GetList(p => (string.IsNullOrEmpty(itemCode) ? true : p.ProdItem == itemCode)
            //                                                    //   && p.PoFlag == "Y"
            //                                                    )
